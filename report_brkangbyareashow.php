...
$strSQL2 = "SELECT send_sup,sale_code FROM st__checkbr WHERE ref_id = '".$ref_id."' ";
...
<form action='save_brkangst1.php' method="post" name="frmMain" enctype="multipart/form-data" onSubmit="JavaScript:return fncSubmit();" >
<body><br>
<center><font size="5">รายงานตรวจเช็คใบยืมค้างเคลียร์ เขตการขาย <?php echo $sale_code; ?> </font></center>
<br>
	<?php if($objResult2["send_sup"]=='0'){ ?>
	<!--a href="sendbr_saleexamine.php?ref_id=<?php echo $_GET["ref_id"];?>&sale_code=<?php echo $sale_code; ?>"  class="w3-button w3-grey w3-right"><font color="red">ส่งรายการให้หัวหน้าอนุมัติ</font></a-->
	
	<a href=javascript:if(confirm('!!!ต้องการส่งข้อมูลรายการสินค้าใบยืมให้หัวหน้าใช่หรือไม่')==true){window.location='sendbr_saleexamine.php?ref_id=<?php echo $_GET["ref_id"];?>&sale_code=<?php echo $sale_code; ?>';}  class="w3-button w3-grey w3-right"><font color="red">ส่งรายการให้หัวหน้าอนุมัติ</font></a>
