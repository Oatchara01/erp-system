...
<div class="w3-panel w3-light-grey"><h3>รายงานแบบสอบถามความพึงพอใจตามสินค้าสาธิต</h3></div>
...
$strSQL2 = "SELECT DISTINCT product_iddemo  FROM tb_research_demo  where 1 ";

if($start_date !=""){ 
    $strSQL2 .= ' AND date_reseach >= "'.$start_date.'"'; 
}

if($end_date !=""){ 
    $strSQL2 .= ' AND date_reseach <= "'.$end_date.'"'; 
}
...
$sql1 = "SELECT sol_name   FROM tb_product where product_ID ='".$objResult2["product_iddemo"]."' ";
...
?><br>
<h7><b><font color='red'><?php echo $rs1["sol_name"]; ?></font></b></h7><br>
...
$strSQL = "SELECT *  FROM tb_research_demo  where product_iddemo = '".$objResult2["product_iddemo"]."' ";
...
<?php 
if(((($objResult["ckk_1"]+$objResult["ckk_2"]+$objResult["ckk_3"]+$objResult["ckk_4"]+$objResult["ckk_5"])/50)*100) < 90) {				?>	<td bgcolor="#FF0000">			
<?php }else{ ?>				
<td>
<?php } ?>	
...
<?php 
if(((($objResult["cs_1"]+$objResult["cs_2"]+$objResult["cs_3"]+$objResult["cs_4"]+$objResult["cs_5"]+$objResult["cs_6"])/60)*100) < 90) {				?>	<td bgcolor="#FF0000">			
<?php }else{ ?>				
<td>
<?php } ?>	<?php  echo number_format((($objResult["cs_1"]+$objResult["cs_2"]+$objResult["cs_3"]+$objResult["cs_4"]+$objResult["cs_5"]+$objResult["cs_6"])/60)*100,2).""; ?> %</td>
...
<td></td><td></td>
<td><?php  echo number_format((($objResult1["ckk_1"]+$objResult1["ckk_2"]+$objResult1["ckk_3"]+$objResult1["ckk_4"]+$objResult1["ckk_5"])/(50*$Num_Rows))*100,2).""; ?> %</td>
		
<td></td>
<td><?php  echo number_format((($objResult1["cs_1"]+$objResult1["cs_2"]+$objResult1["cs_3"]+$objResult1["cs_4"]+$objResult1["cs_5"]+$objResult1["cs_6"])/(60*$Num_Rows))*100,2).""; ?> %</td>
<td></td>
