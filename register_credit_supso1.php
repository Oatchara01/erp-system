...
$strSQL = "SELECT promis_no FROM hos__rental WHERE ref_ai = '".$rs["ref_id"]."' ";
...
(ref_credit,ref_id,date_credit,customer_name,customer_tel,address_name,return_des,send_return_name,date_send_return,sale_name,sale_date,add_by,add_date,ttype_doc,iv_no_ref,sale_code,status_doc,send_sup,company_type,type_return,bank_name,account_name,account_no,book_bank,bill_id,mode_cus,ref_rental)

values

('".$ref_credit."','".$ref_id."','".$date_credit."','".$customer_name."','".$customer_tel."','".$address_name."','".$return_des."','".$send_return_name."','".$date_send_return."','".$sale_name."','".$sale_date."','".$add_by."','".$add_date."','".$ttype_doc."','".$iv_no_ref."','".$sale_code."','".$status_doc."','".$send_sup."','".$company_type."','".$type_return."','".$bank_name."','".$account_name."','".$account_no."','".$_FILES['book_bank']['name']."','".$bill_id."','".$mode_cus."','".$ref_rental."')";
...
echo "alert('บันทึกข้อมูลของท่านเรียบร้อยแล้ว');window.location='register_credit_supedit.php?ref_credit=$ref_credit';";
echo "</script>";
  } else {
