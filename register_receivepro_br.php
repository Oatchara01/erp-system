...
$sql1 = "SELECT *   FROM hos__br where ref_id_br = '".$_GET["ref_id"]."'";
$qry1 = mysqli_query($conn,$sql1) or die(mysqli_error());
$rs1 = mysqli_fetch_assoc($qry1);


$strSQL1 = "SELECT * FROM  (hos__subbr LEFT JOIN tb_product ON hos__subbr.product_ID=tb_product.product_id) WHERE ref_idd_br = '".$_GET["ref_id"]."' ";
...
<input type="radio" name="type_company" checked ='checked' value="2" >&nbsp;NBM
	...
	<div class="w3-bar w3-margin-bottom">
			...
			<input type="hidden" name="ref_iddoc" value = "<?php echo $rs1["ref_id_br"]; ?>" style="width:90%;" class="w3-input"  required>
			<input type="hidden" name="type_doc" value = "<?php echo "BR"; ?>" style="width:90%;" class="w3-input"  required>
...
<div class="w3-bar w3-margin-bottom">
			ชื่อออกบิล
			<input type="text" name="bill_name" id="bill_name" value="<?php echo $rs1["customer"];?>" class="w3-input" style="width:90%;"  required>
</div>

<div class="w3-bar w3-margin-bottom">
ที่อยุ่ออกบิล:<textarea name="bill_address" id="bill_address" class="w3-input" style="width:90%;"  required><?php echo $rs1["address"];?></textarea>
</div>
</div>
