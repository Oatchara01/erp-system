...
$sql = "select * from $table_db  where adm_ckk='1' and demo_ckk='0' and type_company='NBM' and locate('$product_code_search', $find_field) > 0 order by locate('$product_code_search', $find_field), $find_field limit $pagesize";
$result = mysqli_query($conn,$sql);
while ($row = mysqli_fetch_array( $result )) {
	$access_name = $row["access_code"]; // ฟิลที่ต้องการส่งค่ากลับ
