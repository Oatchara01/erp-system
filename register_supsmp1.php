...
echo "alert('ไม่สามารถบันทึกข้อมูลใบเบิกในเดือนนี้ได้เนื่องจากได้ทำการปิดเอกสารเรียบร้อยแล้วค่ะ');window.location='register_supsmp.php';";
echo "</script>";
...
$type_company   = $_POST["type_company"];
	if($sup_name == 'รุจิรา' or $sup_name == 'ลักษณาวรรณ'){
 $allwell_ckk = '1';
	}else{
	$allwell_ckk = '0';	
	}
...
(ref_idsmp,smp_date,address_name,customer_name,comment_sale,status_sup,sale_date,sale_name,sale_code,add_by,add_date,send_sup,sup_name,sup_date,comment_sup,delivery_type,type_company,allwell_ckk,delivery_date,date_send_key,brnp_ckk,brnp_no,up_img1,up_img2,up_img3)
values
('".$ref_idsmp."','".$smp_date."','".$address_name."','".$customer_name."','".$comment_sale."','".$status_sup."','".$sale_date."','".$sale_name."','".$sale_code."','".$add_by."','".$add_date."','".$send_sup."','".$sup_name."','".$sup_date."','".$comment_sup."','".$delivery_type."','".$type_company."','".$allwell_ckk."','".$delivery_date."','".$date_send_key."','".$brnp_ckk."','".$brnp_no."','".$_FILES['up_img1']['name']."','".$_FILES['up_img2']['name']."','".$_FILES['up_img3']['name']."'	)";
