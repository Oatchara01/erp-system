...
document.addEventListener('DOMContentLoaded', function () {
  const paymentSel = document.getElementById('payment_dead');
  const paymentOtherWrap = document.getElementById('payment_dead_other_wrap');
  const paymentOther = document.getElementById('payment_dead_other');

  const deliverySel = document.getElementById('delivery_dead');
  const deliveryDateWrap = document.getElementById('delivery_dead_date_wrap');
  const deliveryDate = document.getElementById('delivery_date');

  function handlePayment() {
    const val = (paymentSel.value || '').trim();
    const isOther = val === 'อื่นๆ' || val.toLowerCase() === 'other';
    paymentOtherWrap.style.display = isOther ? '' : 'none';
    paymentOther.required = isOther;
    if (!isOther) paymentOther.value = '';
  }

  function handleDelivery() {
    const val = (deliverySel.value || '').trim();
    const txt = deliverySel.options[deliverySel.selectedIndex]?.text?.trim() || '';
    const isPickDate = val === 'เลือกวันที่' || txt === 'เลือกวันที่' || val.toLowerCase() === 'pick_date';
    deliveryDateWrap.style.display = isPickDate ? '' : 'none';
    deliveryDate.required = isPickDate;
    if (!isPickDate) deliveryDate.value = '';
  }

  paymentSel.addEventListener('change', handlePayment);
  deliverySel.addEventListener('change', handleDelivery);

  // เรียกครั้งแรกตอนโหลดหน้า (กรณีมีค่าเดิม)
  handlePayment();
  handleDelivery();
});
</script>
...
<div class="w3-button"><input type="radio" checked='checked' name="type_doc"  value="1" id="type_doc">&nbsp; AWL</div><br>
...
<?php include ('register_qou_detail.php')?>
</div> 
