...
<form action='register_adminrental_edit1.php' method="post" name="frmMain" enctype="multipart/form-data" onSubmit="JavaScript:return fncSubmit();" >
...
<input type="radio" checked='checked' name="type_doc" value = "4">&nbsp;NBM
	...
	<?php } ?><br>
	<?php if($objResult["send_acc"]=='2'){ ?>
	<input type="checkbox" name="send_acc" id="send_acc" checked="checked" value="2" >&nbsp;ส่งข้อมูลรับจ่ายบัญชี
	<?php }else{ ?>
	<input type="checkbox" name="send_acc" id="send_acc" value="1" >&nbsp;ส่งข้อมูลรับจ่ายบัญชี
	<?php } ?>
...
ชื่อสินค้า (หลักสัญญา)
<textarea name="head_product" id="head_product" class="w3-input" rows="2" style="width:90%"><?php echo $objResult["head_product"]; ?></textarea>
	
ชื่อสินค้า
<textarea name="des_product" id="des_product" class="w3-input" rows="2" style="width:90%"><?php echo $objResult["des_product"]; ?></textarea>	
</div>
		
<div class="w3-half 1">
	สัญญาเลขที่ : 
<input type="date" name = "promis_date"  value="<?php echo $objResult["promis_date"]; ?>" id="promis_date" style="width:90%;" class = "w3-input"> 
<input type="hidden" name = "admin_name"  value="<?php echo $objResult["admin_name"]; ?>" id="admin_name" style="width:90%;" class = "w3-input"> 	
...
ชื่อสินค้า(ข้อ 1 ทรัพย์ที่เช่า)
<textarea name="des_productunit" id="des_productunit" class="w3-input" rows="3" style="width:90%"><?php echo $objResult["des_productunit"]; ?></textarea>	
...
<a class="w3-bar-item w3-button" onclick="openCity1('cs1')"><font color="#404040"><b>เอกสารเพิ่มเติม</b></font></a>

	
</div>

<div id="cs2" class="w3-container city1" >
<br>
...
$strSQL_15 = "SELECT SUM(amount) AS amount FROM hos__subso WHERE ref_idd = '".$objResult["ref_ai"]."' ";
$objQuery_15 = mysqli_query($conn,$strSQL_15);
$objResult_15= mysqli_fetch_array($objQuery_15);
$amount__n = $objResult_15["amount"];
...
$strSQL_15 = "SELECT SUM(sum_amount) AS amount FROM so__submain WHERE ref_idd = '".$objResult["ref_ai"]."' ";
$objQuery_15 = mysqli_query($conn,$strSQL_15);
$objResult_15= mysqli_fetch_array($objQuery_15);
$amount__n = $objResult_15["amount"];
...
<th width="10%"><?php echo number_format($amount__n,2).""; ?></th>	
...
$ref_ai = str_replace(" ", "", $objResult["ref_ai"]);		
if($ref_ai!=''){		
$strSQLd2 = "SELECT *  FROM tb_credit_note  where ref_id = '".$ref_ai."'";	
$objQueryd2 = mysqli_query($conn,$strSQLd2) or die ("Error Query [".$strSQLd2."]");
$Num_Rowsd2 = mysqli_num_rows($objQueryd2);
$objResultd2= mysqli_fetch_array($objQueryd2);		
if($Num_Rowsd2 > 0){
	
$strSQL_d15 = "SELECT SUM(sum_amount) AS sum_amount FROM tb_subcredit WHERE ref_creditt = '".$objResultd2["ref_credit"]."' ";
$objQuery_d15 = mysqli_query($conn,$strSQL_d15);
$objResult_d15= mysqli_fetch_array($objQuery_d15);
$amount__dn = $objResult_d15["sum_amount"];
	
	
	
?>
<tr>		
<th width="10%">0.ลดหนี้</th>
<th width="10%"><?php echo Datethai($objResultd2["date_credit"]); ?></th> 
<th width="10%"></th>
<th width="23%">
	<a href="register_credit_adm.php?ref_credit=<?php echo $objResultd2["ref_credit"]; ?>" target="_blank">
     <?php echo $objResultd2["credit_no"]; ?>
    </a>
	
	</th>
<th width="10%"><?php echo number_format($amount__dn,2).""; ?></th>	
<th width="22%"><?php if($Num_Rows18 > 0){ if($objResult18["date_tran"]!='0000-00-00'){ echo "คืนเงินค้ำประกันเรียบร้อย วันที่ ";  echo Datethai($objResult18["date_tran"]); } } ?></th>		
</tr>		
...
$strSQL_15 = "SELECT SUM(amount) AS amount FROM hos__subso WHERE ref_idd = '".$objResult2["ref_idiv"]."' ";
$objQuery_15 = mysqli_query($conn,$strSQL_15);
$objResult_15= mysqli_fetch_array($objQuery_15);
$amount__n2 = $objResult_15["amount"];
...
$strSQL_15 = "SELECT SUM(sum_amount) AS amount FROM so__submain WHERE ref_idd = '".$objResult2["ref_idiv"]."' ";
$objQuery_15 = mysqli_query($conn,$strSQL_15);
$objResult_15= mysqli_fetch_array($objQuery_15);
$amount__n2 = $objResult_15["amount"];
...
<th width="10%"><?php
	...
	//$new_date = date('Y-m-d', strtotime($date_runiv . ' +30 days'));
	echo Datethai($objResult2["end_date"]); ?></th>
<th width="23%">
  <?php if ($ref_iv == 'SO') { ?>
...
<th width="10%"><?php echo number_format($amount__n2,2).""; ?></th>		
...
$strSQL_15 = "SELECT SUM(amount) AS amount FROM hos__subso WHERE ref_idd = '".$objResult["ref_iv"]."' ";
$objQuery_15 = mysqli_query($conn,$strSQL_15);
$objResult_15= mysqli_fetch_array($objQuery_15);
$amount__n1 = $objResult_15["amount"];
...
$strSQL_15 = "SELECT SUM(sum_amount) AS amount FROM so__submain WHERE ref_idd = '".$objResult["ref_iv"]."' ";
$objQuery_15 = mysqli_query($conn,$strSQL_15);
$objResult_15= mysqli_fetch_array($objQuery_15);
$amount__n1 = $objResult_15["amount"];
...
<th width="10%"><?php echo number_format($amount__n1,2).""; ?></th>			
...
<div id="pd" class="w3-container city1" style="display:none">
...
<br>
<a href="promis_from.php?ref_id=<?php echo $objResult["ref_id"];?>" target="_blank" class="w3-button w3-grey w3-center"><font color="purple">หนังสือสัญญา</font></a>	
...
<?php if ($objResult["job_no"]!=''){ ?>
		
<a href="https://cs.allwellcenter.com/7112018.php?running=<?php echo $objResult["job_no"];?>" target="_blank" class="w3-button w3-grey w3-right"><font color="Blue">รายละเอียดจัดส่ง</font></a><br />
...
<br>
<center>
<?php //if($objResult["stock_print"]!=''){ }else{ ?>
