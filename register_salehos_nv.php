    ...
    if (xhr.readyState === 4) {
      if (xhr.status === 200) {
        try {
          ...
          // 🔥 Popup + Redirect เมื่อวงเงินติดลบ
         if (remain < 0) {
    Swal.fire({
        ...
        html:
          "<div style='font-size:16px; text-align:left;'>" +
            ...
            "<hr>" +
            "<p><b>วงเงิน:</b> " + numberFormat(credit) + " บาท</p>" +
...
<input type='text' name = "bill_id"  id = "bill_id" class="button4" placeholder="Search ชื่อลูกค้า..."  style="width:30%;" OnChange="JavaScript:doCallAjax1('bill_id','bill_name','bill_address','bill_tel','tax_id','pre_name','mode_name','email','customer_typename','payment','credit_thb');" required> 
<input type='hidden' name = "h_bill_id"  id = "h_bill_id"  class="button4" readonly>
<input type='hidden' name = "mode_name"  id = "mode_name"  class="button4" readonly></p>
