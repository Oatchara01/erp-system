...
$sql58 = "SELECT credit_ckk,credit_thb FROM tb_customer where customer_id  = '".$objResult["customer_id"]."'";
...
$sql60 = "SELECT 
          ...
          WHERE summary_cash <> 'สมบูรณ์'
            ...
            AND bill_id = '".$objResult["customer_id"]."'";
$qry60 = mysqli_query($code,$sql60) or die(mysqli_error($code));
$rs60  = mysqli_fetch_assoc($qry60);

// ดึงยอดรับชำระ (amount)
$sql61 = "SELECT COALESCE(SUM(amount),0) AS amount FROM tb_receipt_cash WHERE ref_id_off = '".$rs60["id_off"]."'";
...
if (remain < 0 && creditThb !== 0) {
        ...
        Swal.fire({
            ...
            window.location.href = "status_clearbr.php";
        });
    }
})();
</script>
	...
	<form action='rister_clearbrpn1.php' method="post" name="frmMain" enctype="multipart/form-data">
	<div class="w3-white">
...
<?php
  // ดึงค่ามาให้ชัด
  $bill_id = isset($objResult['customer_id']) ? trim((string)$objResult['customer_id']) : '';
?>
<?php if ($rs59['pay_in'] !== ''): ?>
