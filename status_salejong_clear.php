...
<div class="w3-panel w3-light-gray"><h4>Status ใบจอง (ค้างเคลียร์)</h4></div>
...
$strSQL = "SELECT *  FROM hos__jongproduct  where sale_code LIKE '%EN%' and close_jong = '0' and cancel_ckk = '0' and status_doc='Approve'";
}else{
$strSQL = "SELECT *  FROM hos__jongproduct  where sale_code = '".$sale_code."' and close_jong = '0' and cancel_ckk = '0' and status_doc='Approve'";	
}
...
if($Keyword !=""){ //แสดงว่ามีค่า end_date ส่งมา หรือมีการค้นหา ก็ใหเต่อ String query
	$strSQL .= ' AND (customer  LIKE "%'.$Keyword.'%" OR iv_no LIKE "%'.$Keyword.'%"  OR ref_id LIKE "%'.$Keyword.'%")'; 
...
<div class="w3-container">
	<table border="1" width="100%" class="w3-table">
		<thead class="w3-gray">
			...
			<th width="8%">เปิดใบสั่งขาย</th>
			<th width="8%">ยกเลิกใบจอง</th>
...
echo DateThai($objResult["date_receive"]);
				...
				<td><div align="left">
					<?php
						$strSQL1 = "SELECT * FROM (hos__subjongpro LEFT JOIN tb_product ON hos__subjongpro.product_ID=tb_product.product_id) WHERE ref_idd = '".$objResult["ref_id"]."' ";
						//echo $strSQL1;
						//exit();
	...
	?><br />
				...
				<td><div align="left"><?php echo $objResult["drescription"];?></div></td>
			<td><div align="left"><?php echo $objResult["sale_code"];?> <?php echo '-'; ?> <?php echo $objResult["sale_name"];?></div></td>
				...
				<?php } ?>
				<td>
				<a href="register_salebook_edit.php?ref_id=<?php echo $objResult["ref_id"];?>&start_date=<?php echo $_GET["start_date"];  ?>&end_date=<?php echo $_GET["end_date"];?>"><img src="img/edit-icon.png" width="23" height="23" border="0" /></a>
											
				</td>
					<td>
						<a href=javascript:if(confirm('!!!ต้องการเปิดเอกสารใบสั่งขายใช่หรือไม่')==true){window.location='register_salejong_create.php?ref_id=<?php echo $objResult["ref_id"];?>';}><img src="img/sticker.png" width="23" height="23" border="0" /></a>
									
					</td>
				
<td>
						<a href=javascript:if(confirm('!!!ต้องการยกเลิกใบจองใช่หรือไม่')==true){window.location='cancel_jong.php?ref_id=<?php echo $objResult["ref_id"];?>';}><img src="img/sticker.png" width="23" height="23" border="0" /></a>
