...
<div class="w3-container w3-white">
	...
	<h3>Incomplete Delivery</h3>
		...
		<?php if($objResult["status_doc"]=='Request'){ ?>			
<a href="send_admapprove.php?ref_id=<?php echo $objResult["ref_id"];?>" target="_blank" class="w3-button w3-yellow w3-right"><font color="red">ปิดงาน</font></a>&nbsp;&nbsp;
									  <?php } ?>
	</div></div>
	<form action='register_noadapprove1.php' method="post" name="frmMain" enctype="multipart/form-data">
	<div class="w3-bar">
...
<div class="w3-bar w3-margin-bottom">
	...
	<div class="w3-bar w3-margin-bottom">
			<span>ลงงานจัดส่ง IP</span> <input type="text" name="delivery_no" id="delivery_no" value = "<?php echo $objResult["delivery_no"]; ?>" class="w3-input" style="width:90%;" >
		</div>
	
	<div class="w3-bar w3-margin-bottom">
			<span>ชื่อผู้แก้ไข</span> <input type="text" name="editor_name" id="editor_name" value = "<?php echo $objResult["editor_name"]; ?>" class="w3-input" style="width:90%;" >
		</div>
...
<div class="w3-bar w3-margin-bottom">
	...
	<div class="w3-bar w3-margin-bottom">
			...
			<div class="w3-bar w3-margin-bottom">
			<span>วันที่แก้ไข</span> <input type="text" name="date_editor" id="date_editor" value = "<?php echo $objResult["date_editor"]; ?>" class="w3-input" style="width:90%;" readonly>
		</div>		
...
<fieldset><legend ><b><font color="red">ส่วนเพิ่มเติมการรับเรื่อง</font></b></legend></p>
		...
		<div class="w3-half ">
			...
			<div class="w3-bar w3-margin-bottom">
				การประเมิน สาเหตุเกิดจาก
				</div>
			
				<div class="w3-bar w3-margin-bottom">
					<?php if($objResult["dept_en"]=='1'){ ?>
					<input type="checkbox" name="dept_en" checked="checked"  id="dept_en" value="1"> แผนกวิศวกรรม
					<?php }else{ ?>
					<input type="checkbox" name="dept_en"  id="dept_en" value="1"> แผนกวิศวกรรม
					<?php } ?>
						</div>
			
			<div class="w3-bar w3-margin-bottom">
					<?php if($objResult["dept_st"]=='1'){ ?>
					<input type="checkbox" name="dept_st" checked="checked"  id="dept_st" value="1"> แผนกคลังสินค้า
					<?php }else{ ?>
					<input type="checkbox" name="dept_st"  id="dept_st" value="1"> แผนกคลังสินค้า
					<?php } ?>
						</div>
			
			<div class="w3-bar w3-margin-bottom">
					<?php if($objResult["dept_cs"]=='1'){ ?>
					<input type="checkbox" name="dept_cs" checked="checked"  id="dept_cs" value="1"> แผนกจัดส่ง
					<?php }else{ ?>
					<input type="checkbox" name="dept_cs"  id="dept_cs" value="1"> แผนกจัดส่ง
					<?php } ?>
						</div>
			
			<div class="w3-bar w3-margin-bottom">
					<?php if($objResult["dept_sale"]=='1'){ ?>
					<input type="checkbox" name="dept_sale" checked="checked"  id="dept_sale" value="1"> ฝ่ายขาย
					<?php }else{ ?>
					<input type="checkbox" name="dept_sale"  id="dept_sale" value="1"> ฝ่ายขาย
					<?php } ?>
						</div>
			<div class="w3-bar w3-margin-bottom">
					<?php if($objResult["dept_adm"]=='1'){ ?>
					<input type="checkbox" name="dept_adm" checked="checked"  id="dept_adm" value="1"> ฝ่ายสำนักงาน
					<?php }else{ ?>
					<input type="checkbox" name="dept_adm"  id="dept_adm" value="1"> ฝ่ายสำนักงาน
					<?php } ?>
						</div>
			<div class="w3-bar w3-margin-bottom">
			<span>ข้อเสนอแนะ</span> <textarea type="text" name="description"  id="description" class="w3-input" style="width:90%;" rows="2" required><?php echo $objResult["description"]; ?></textarea>
		</div>
			</div>
		
		<div class="w3-half ">
			<div class="w3-bar w3-margin-bottom">
				ข้อสรุปการแก้ไขหรือวิธีป้องกัน
				</div>
			
				<div class="w3-bar w3-margin-bottom">
					<?php if($objResult["per"]=='1'){ ?>
					<input type="checkbox" name="per" checked="checked"  id="per" value="1"> PER  เลขที่
					<?php }else{ ?>
					<input type="checkbox" name="per"  id="per" value="1"> PER  เลขที่
					<?php } ?>
				<input type="text" name="per_no"  value="<?php echo $objResult["per_no"]; ?>" id="per_no" class="w3-input" style="width:90%;" >
						</div>
			
				<div class="w3-bar w3-margin-bottom">
					<?php if($objResult["smp"]=='1'){ ?>
					<input type="checkbox" name="smp" checked="checked"  id="smp" value="1"> SMP  เลขที่
					<?php }else{ ?>
					<input type="checkbox" name="smp"  id="smp" value="1"> SMP  เลขที่
					<?php } ?>
				<input type="text" name="smp_no"  value="<?php echo $objResult["smp_no"]; ?>" id="smp_no" class="w3-input" style="width:90%;" >
						</div>
			
				<div class="w3-bar w3-margin-bottom">
					<?php if($objResult["spr"]=='1'){ ?>
					<input type="checkbox" name="spr" checked="checked"  id="spr" value="1"> SPR  เลขที่
					<?php }else{ ?>
					<input type="checkbox" name="spr"  id="spr" value="1"> SPR  เลขที่
					<?php } ?>
				<input type="text" name="spr_no"  value="<?php echo $objResult["spr_no"]; ?>" id="spr_no" class="w3-input" style="width:90%;" >
						</div>
			
				<div class="w3-bar w3-margin-bottom">
					<?php if($objResult["chang"]=='1'){ ?>
					<input type="checkbox" name="chang" checked="checked"  id="chang" value="1"> ใบแลกเปลี่ยน  เลขที่
					<?php }else{ ?>
					<input type="checkbox" name="chang"  id="chang" value="1"> ใบแลกเปลี่ยน  เลขที่
					<?php } ?>
				<input type="text" name="chang_no"  value="<?php echo $objResult["chang_no"]; ?>" id="chang_no" class="w3-input" style="width:90%;" >
						</div>
			
				<div class="w3-bar w3-margin-bottom">
					<?php if($objResult["par"]=='1'){ ?>
					<input type="checkbox" name="par" checked="checked"  id="par" value="1"> PAR  เลขที่
					<?php }else{ ?>
					<input type="checkbox" name="par"  id="par" value="1"> PAR  เลขที่
					<?php } ?>
				<input type="text" name="par_no"  value="<?php echo $objResult["par_no"]; ?>" id="par_no" class="w3-input" style="width:90%;" >
						</div>
			
				<div class="w3-bar w3-margin-bottom">
					<?php if($objResult["car"]=='1'){ ?>
					<input type="checkbox" name="car" checked="checked"  id="car" value="1"> CAR  เลขที่
					<?php }else{ ?>
					<input type="checkbox" name="car"  id="car" value="1"> CAR  เลขที่
					<?php } ?>
				<input type="text" name="car_no"  value="<?php echo $objResult["car_no"]; ?>" id="car_no" class="w3-input" style="width:90%;" >
						</div>
