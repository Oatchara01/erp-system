...
<div class="w3-white w3-container">
	...
	<form action="register_breng1.php" method="post" name="frmMain" enctype="multipart/form-data" 
...
<?php if($_GET['service_order_no'] !=''){ ?> <input type="hidden" name="service_order_no" id="service_order_no" value="<?php echo $_GET['service_order_no'];?>"> <?php } ?> <!-- อ้างอิงไปยังใบงานบริการ -->

	<div class="w3-bar">
		<input type="radio" name="company" value="1" checked='checked' required>ใบยืม AWL <br>
		
        <input type="radio" name="type_breng" value="0"  required> ใบยืมลูกค้า (BRNP)
		<input type="radio" name="type_breng" value="1"  required> ใบยืมช่าง (BRES)
		<!-- <input type="radio" name="type_breng" value="2"  required> ใบยืมสินค้าตรวจเช็ค (BREQ) -->
...
// ดึงค่า จาก URL parameters
let urlParams = new URLSearchParams(window.location.search);
let service_order_no = urlParams.get('service_order_no');
let hospital_name = urlParams.get('hospital_name');
console.log(service_order_no);
console.log(hospital_name);

document.getElementById('cm_no').value = service_order_no;
