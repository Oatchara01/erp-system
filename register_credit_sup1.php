...
echo "alert('ไม่สามารถบันทึกข้อมูลใบลดหนี้ในเดือนนี้ได้เนื่องจากได้ทำการปิดเอกสารเรียบร้อยแล้วค่ะ');window.location='register_credit_sup.php';";
echo "</script>";
	
}else{
	
$customer_name = $_POST["customer_name"];
$bill_id = $_POST["customer_id"];
...
$temp = explode(".", $_FILES["book_bank"]["name"]);
$book_bank = "book_bank" . "_" . $ref_credit . "_" . round(microtime(true)) . '.' . end($temp);
...
(ref_credit,date_credit,customer_name,customer_tel,address_name,return_des,send_return_name,date_send_return,sale_name,sale_date,add_by,add_date,ttype_doc,iv_no_ref,sale_code,status_doc,send_sup,company_type,type_return,bank_name,account_name,account_no,book_bank,bill_id,mode_cus)
values
('".$ref_credit."','".$date_credit."','".$customer_name."','".$customer_tel."','".$address_name."','".$return_des."','".$send_return_name."','".$date_send_return."','".$sale_name."','".$sale_date."','".$add_by."','".$add_date."','".$ttype_doc."','".$iv_no_ref."','".$sale_code."','".$status_doc."','".$send_sup."','".$company_type."','".$type_return."','".$bank_name."','".$account_name."','".$account_no."','".$book_bank."','".$bill_id."','".$mode_cus."')";
