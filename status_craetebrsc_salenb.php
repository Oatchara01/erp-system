...
<div class="w3-panel w3-light-grey"><h3>Status ใบยืมค้ฝากขายางเคลียร์ (NBM)</h3></div>
...
<input type='hidden' name = "type_doc" value="<?php echo"2"; ?>" id = "type_doc" class="w3-input" />	
...
<input type="submit" class="w3-button w3-teal" value="Search"></div>
 ...
 <center>
		  <input type="button" name ="Submit" value="เปิดใบสั่งขาย" class = "w3-button w3-purple" onClick="this.form.action='register_salehos_brscany.php'; submit()">
		</center><br>
<?php	
...
$strSQL = "SELECT ref_id,iv_no,iv_date,customer,sale_code,status_doc,sale,date_save  FROM hos__consig  where sale_code LIKE '%EN%' and close_br = '0' and status_doc = 'Approve' and company='1'";
}else{
$strSQL = "SELECT ref_id,iv_no,iv_date,customer,sale_code,status_doc,sale,date_save FROM hos__consig  where  close_br ='0' and status_doc ='Approve' and sale_code ='".$sale_code."' and company='1'";	
}
...
$strSQL1 = "SELECT product_id,count,sn,sale_remark FROM hos__subconsig WHERE ref_idd='".$objResult["ref_id"]."' and clear_ckk='0' and ckk_st='1'";
...
$sql4 = "SELECT sum(count) as count4   FROM  hos__subreceive  where ref_idd = '".$rs41['ref_id']."' and product_id = '".$objResult['product_id']."'";
...
function make_autocom(autoObj,showObj){
	...
	new Autocomplete(mkAutoObj, function() {
		...
		if ( this.value.length < 1 && this.isNotClick ) 
			return ;	
		return "data_pro_notdemonbeng5.php?product_code_search=" +encodeURIComponent(this.value);
    });	
}	
...
function make_autocom(autoObj,showObj){
	...
	new Autocomplete(mkAutoObj, function() {
		...
		if ( this.value.length < 1 && this.isNotClick ) 
			return ;	
		return "data_pro_notdemothnbeng5.php?product_code_search=" +encodeURIComponent(this.value);
    });	
}	
...
function make_autocom(autoObj,showObj){
	...
	new Autocomplete(mkAutoObj, function() {
		...
		if ( this.value.length < 1 && this.isNotClick ) 
			return ;	
		return "data_pro_notdemonb5.php?product_code_search=" +encodeURIComponent(this.value);
    });	
}	
...
function make_autocom(autoObj,showObj){
	...
	new Autocomplete(mkAutoObj, function() {
		...
		if ( this.value.length < 1 && this.isNotClick ) 
			return ;	
		return "data_pro_notdemothnb5.php?product_code_search=" +encodeURIComponent(this.value);
    });	
}	
