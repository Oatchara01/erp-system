...
$strSQL2 = "SELECT send_stock,sale_code FROM st__checkbr WHERE ref_id = '".$ref_id."' ";
...
<form action='save_brkangstsup1.php' method="post" name="frmMain" enctype="multipart/form-data" onSubmit="JavaScript:return fncSubmit();" >
<body><br>
<center><font size="5">รายงานตรวจเช็คใบยืมค้างเคลียร์ เขตการขาย <?php echo $objResult2["sale_code"]; ?> </font></center>
<br>
	<?php if($objResult2["send_stock"]=='0'){ ?>
	
	<!--a href="sendbr_supexamine.php?ref_id=<?php echo $_GET["ref_id"];?>&sale_code=<?php echo $objResult2["sale_code"]; ?>"  class="w3-button w3-grey w3-right"><font color="red">ส่งรายการให้ Stock</font></a-->
	
	<a href=javascript:if(confirm('!!!ต้องการส่งข้อมูลรายการสินค้าใบยืมให้Stockใช่หรือไม่')==true){window.location='sendbr_supexamine.php?ref_id=<?php echo $_GET["ref_id"];?>&sale_code=<?php echo $sale_code; ?>';}  class="w3-button w3-grey w3-right"><font color="red">ส่งรายการให้ Stock</font></a>
...
$strSQL9 = "SELECT id,iv_no,stock_date,customer_name,product_code,count,iv_date,sn_number,have_ckk,des_sale  FROM st__checkbr WHERE  ref_id = '".$ref_id."'";
