...
function doCallAjax1(bill_id,email,billing_name,billing_address,billing_tel,tax_id,ex_add,ex_aumper,ex_provin,ex_post,pre_name,customer_name,address1,address2,province,postcode,tel,delivery_contact,delivery_place,customer_typename) {
HttPRequest = false;
...
<body>
	...
	<br>
	<center>
ค้นหา : <input name="Keyword"  style="width:10%;" type="text" id="Keyword" value="<?php echo $Keyword = isset($_POST['Keyword']) ? $_POST['Keyword'] : '';?>">

<input type="button" name ="" value="ค้นหา" class = "w3-button w3-grey w3-center" onClick="this.form.action='register_admin_edit.php'; submit()">

<br>
<input type="submit" name ="" value="บันทึก" class = "w3-button w3-green w3-center" onClick="this.form.action='register_admin_edit1.php'; submit()">
...
if($Keyword !=''){
		...
		$strSQL = "SELECT so__main.* ,tb_salechannel.* ,tb_delivery.* ,tb_payment.*  FROM (((so__main LEFT JOIN tb_salechannel ON so__main.sale_channel=tb_salechannel.salechannel_ID) LEFT JOIN tb_delivery ON so__main.delivery=tb_delivery.delivery_id) LEFT JOIN tb_payment ON so__main.payment=tb_payment.payment_ID) WHERE ref_id = '".$Keyword."' or customer_name  LIKE '%".$Keyword."%' or tel  LIKE '%".$Keyword."%' or order_id  LIKE '%".$Keyword."%' or delivery_contact LIKE '%".$Keyword."%'";


		$objQuery = mysqli_query($conn,$strSQL) or die(mysqli_error());
		$objResult = mysqli_fetch_array($objQuery);

$strSQL1 = "SELECT * FROM  (so__submain LEFT JOIN tb_product ON so__submain.product_ID=tb_product.product_id) WHERE ref_idd = '".$Keyword."' ";
...
SELECT EXISTS(
    ...
    WHERE s.ref_idd = '".$ref_id."'
      AND (
         ...
         OR ((p.group1 LIKE '%รถเข็น%' AND s.product_ID IN (6016,6015,6014,6013,6012)) AND p.unit_name = 'คัน')
...
$sql_check = "
    SELECT COUNT(*) AS total_other
    FROM so__submain s
    LEFT JOIN tb_product p ON s.product_ID = p.product_id
    WHERE s.ref_idd = '".$ref_id."'
      AND p.product_type = 'อื่นๆ'
";
...
if ($row_total['total_item'] == 1 && $row_check['total_other'] == 1) {

    mysqli_query($conn, "
        UPDATE so__main 
        SET ckk_h = '1',send_erpst='1' 
        WHERE ref_id = '".$ref_id."'
    ");
...
<div class="w3-container w3-padding-large"><!-- main div -->
		...
		<div class="w3-half">
				...
				<div class="w3-half">
					...
					<a href="send_showroom_edit.php?ref_id=<?php echo $objResult["ref_id"];?>" class="w3-button w3-grey w3-right"><font color="blue">ส่งข้อมูลกลับให้ Sale แก้ไข</font></a>
					<?php } ?>
					<a href="send_receive.php?ref_id=<?php echo $objResult["ref_id"];?>" class="w3-button w3-grey w3-right"><font color="330066">ส่งข้อมูลไปรับจ่ายบัญชี</font></a>
					...
					<?php if($objResult["send_supadm"]=='0'){ ?>			
<a href="send_supadm.php?ref_id=<?php echo $objResult["ref_id"];?>&billing_name=<?php echo $objResult["billing_name"];?>" target="_blank" class="w3-button w3-grey w3-right"><font color="red">ส่งใบกำกับภาษีให้ Sup อนุมัติ</font></a>&nbsp;&nbsp;	
...
<?php } ?>
	...
	<br><br><b>หมายเหตุแจ้งแผนกที่เกี่ยวข้อง</b>	
<br><br>	
จัดส่ง :
...
<input type="checkbox" name="run_et" value="1"> &nbsp;Run เลขที่เอกสาร ET<br>
<input type="checkbox" name="run_brnp" value="1"> &nbsp;Run เลขที่เอกสาร BRNP				
<?php } ?>	
...
<a href="report_brnpsol_awl.php?ref_id=<?php echo $objResult["ref_id"];?>" target="_blank" class="w3-button w3-grey"><font color="Blue">ใบยืมต่อเนื่อง (New)</font></a>&nbsp;&nbsp;
...
<a href="report_brnpsol_awl.php?ref_id=<?php echo $objResult["ref_id"];?>" target="_blank" class="w3-button w3-grey"><font color="Blue">ใบยืมต่อเนื่อง (New)</font></a>&nbsp;&nbsp;
				...
				<?php }
					if ($objResult["select_type_doc"]=='3'){ ?>
						...
						<a href="report_saleptl1.php?ref_id=<?php echo $objResult["ref_id"];?>" target="_blank" class="w3-button w3-grey"><font color="Blue">ใบสั่งขาย AWL</font></a><br/>
				<?php }
				else { ?>
...
<br>
<?php  if($objResult["sale_channel"]=='34'){ ?>

<a href="report_loanptl_sum.php?doc_no=<?php echo $objResult["doc_no"];?>" target="_blank" class="w3-button w3-grey"><font color="Blue">ใบยืม Tiktok</font></a>&nbsp;&nbsp;
<?php } ?>
			</div>
	...
	<div id="so" class="w3-container city" style="display:none"> 
		<div class="w3-half"><!--first half-->
			<div class="w3-bar">
				...
				รหัสลูกค้า
<input type='text' name = "bill_id"  id = "bill_id" value="<?php echo $objResult['bill_id']; ?>" class="w3-input" placeholder="Search ชื่อลูกค้า..."   OnChange="JavaScript:doCallAjax1('bill_id','email','billing_name','billing_address','billing_tel','tax_id','ex_add','ex_aumper','ex_provin','ex_post','pre_name','customer_name','address1','address2','province','postcode','tel','delivery_contact','delivery_place','customer_typename');" > 
...
<input type="text" name="email" id="email" class="w3-input" value="<?php echo $objResult['email']; ?>" >	
			</div>						
<div class="w3-bar w3-half">
...
<input type="text" name="tax_id" id="tax_id" class="w3-input" value="<?php echo $objResult['tax_id']; ?>" >
		...
		<div class="w3-half w3-container w3-light-grey"><!--second half-->
			...
			ที่อยู่ในการจัดส่ง
			<input name="address1" id="address1" class="w3-input" type="text" value="<?php echo $objResult['address1'];?>">
			<input name="address2" id="address2" class="w3-input" type="text" value="<?php echo $objResult['address2'];?>">
			จังหวัด
...
<div id="so4" class="w3-container city" style="display:none">
	...
	<div id="mo" class="w3-container city" style="display:none">
		<div class="w3-third"><!-- 1st 3rd -->
			...
			กำหนดส่งตามสัญญา
			<input name="delivery_contract" id="delivery_contract" type='date' class="w3-input" value="<?php echo $objResult["delivery_contract"];?>" >
			<?php
...
<div id="lg" class="w3-container city1" style="display:none">
	<div class="w3-half">
		<div class="w3-bar">
		วันที่คีย์ค่าจัดส่ง
<input name="date_ker" class="w3-input" value="<?php echo $objResult['date_ker']; ?>"  type="date" >
		</div>
		
	<div class="w3-bar">
		รหัสอ้างอิงการส่ง1
<input name="order_refer_code" class="w3-input" value="<?php echo $objResult['order_refer_code']; ?>"  type="text" >
		</div>
		
		<div class="w3-bar">
		รหัสอ้างอิงการส่ง2
<input name="order_refer_code1" class="w3-input" value="<?php echo $objResult['order_refer_code1']; ?>"  type="text" >
		</div>
		
		<div class="w3-bar">
		ค่าจัดส่ง
<input name="ker_bath" class="w3-input" value="<?php echo $objResult['ker_bath']; ?>"  type="text" >
		</div>
		</div>
...
<thead>
	...
	<th><div class="w3-center"><b>หมายเลขเครื่อง</b></div></th>
	 <th><div class="w3-center"><b>โค้ดส่วนลด</b></div></th>
    <th><div class="w3-center"><b>หมายเหตุ</b></div></th>
	<th><div class="w3-center"><b>เคลียร์ IV</b></div></th>
	<th><div class="w3-center"><b>เคลียร์ยืม</b></div></th>
	<th><div class="w3-center"><b>เคลียร์จอง</b></div></th>
...
$strSQL2 = "SELECT product_code,sale_count  FROM (so__submain LEFT JOIN tb_product ON so__submain.product_ID=tb_product.product_id) where ref_idd = '".$ref_id."' and product_code ='".$objResult1["product_code"]."' and ckk_pro='0'  and review_ckk='1' and type_br ='1'";
...
$count = str_replace('.00','',$objResult2["sale_count"]);
...
values
('".$ref_pc."','".$doc_no."','".$strYear1."','".$ref_id."','".$product_id."','".$add_date."','".$add_by."','".$strDate."','PC','1')";
...
$strSQL = "Update   so__submain set ckk_pro='1'  Where ref_idd = '".$ref_id."' and product_id ='".$product_id."'";
...
<td style="width:10%;"><textarea name = "code_dis[<?php echo $objResult1["id"];?>]"   id = "code_dis[<?php echo $objResult1["id"];?>]"  class="w3-input" ><?php echo $objResult1["code_dis"];?></textarea></td>
<td>
...
<td>
<textarea  name = "clear_ivno1[<?php echo $objResult1["id"];?>]"  id = "clear_ivno1[<?php echo $objResult1["id"];?>]"  class="w3-input" ><?php echo $objResult1["clear_ivno1"];?></textarea>
...
<?php
/*if($objResult["send_erpst"]=='0'){*/
	 if ($objResult["select_type_doc"]=='1' or $objResult["select_type_doc"]=='3'){ 
include ('data_product_adedit.php');
	 }else  if ($objResult["select_type_doc"]=='2' or $objResult["select_type_doc"]=='4' ){ 
	include ('data_product_adeditnb.php');
	 }
	/*}*/
?>
...
<?php
	...
	<input type="checkbox" name="bus_inter"  value="1" checked ='checked' id="bus_inter" >ขนส่งอินเตอร์ <br />
	<?php }else{ ?>
...
<?php } ?>
		 <input name="unit_cash" type='text' class="w3-input" id="unit_cash" value="<?php echo $fetch1["price"]; ?>" size="20" rows="1" style="color:black;text-align:right" OnChange="JavaScript:chkNum(this)">  
		 </div><div class="w3-container w3-third">
...
<input type="checkbox"  id = "credit" name="credit" checked="checked" value="1">รูดการ์ด 
<?php }else { ?>
<input type="checkbox"  id = "credit" name="credit" value="1">รูดการ์ด 
...
<div class="w3-container w3-third">
รายละเอียดเพิ่มเติม : 
     <textarea name="description"  class="w3-input" id="description" cols="54" rows="3"><?php echo $fetch1["description"]; ?></textarea>
</div>
