...
$strSQL6 = "Update   so__submain set ckk_free='1'   Where  ref_idd='".$fetch21["ref_id"]."' and sku_code='".$sku."'";
$objQuery6 = mysqli_query($conn,$strSQL6);		

	
} else if($sku=='4987067923809' or $sku=='4987067923908' or $sku=='4987067924004' or $sku=='4987067923106' or $sku=='4987067923205' or $sku=='4987067923304'  or $sku=='4987067140909' or $sku=='4987067141005' or $sku=='4987067141104'  or $sku=='4987067141807' or $sku=='4987067141906' or $sku=='4987067142002'  or $sku=='SUP-KOW-CFKNEM' or $sku=='SUP-KOW-CFKNEL' or $sku=='4987067923601'){

$strSQL6 = "Update   so__submain set ckk_free='2'   Where  ref_idd='".$fetch21["ref_id"]."' and sku_code='".$sku."'";
$objQuery6 = mysqli_query($conn,$strSQL6);		
...
/*if($sku=='GLS25' or $sku=='GLS50' or $sku=='GLS100'  or $sku=='GLS150'  or $sku=='GLS200'  or $sku=='GLS252P'  ){

$strSQL19 = "insert into so__submain
(ref_idd,sale_count,sale_countref,price_per_unit,price_per_unitref,sum_amount,discount_unit,product_id,product_code,warranty,status_sol,sale_remark)
values ('".$fetch21["ref_id"]."','1','1','0.00','0.00','0.00','0.00','5638','5638','','Approve','Pro พิเศษ แถม Glucerna SR กลูเซอนา เอสอาร์ กลิ่นวานิลลา 52.1g สำหรับผู้ป่วยเบาหวาน')";
$objQuery19 = mysqli_query($conn,$strSQL19);
	
}	

if($sku=='GCA10' or $sku=='GCA60'){

$strSQL19 = "insert into so__submain
(ref_idd,sale_count,sale_countref,price_per_unit,price_per_unitref,sum_amount,discount_unit,product_id,product_code,warranty,status_sol,sale_remark)
values ('".$fetch21["ref_id"]."','1','1','0.00','0.00','0.00','0.00','5637','5637','','Approve','Pro พิเศษ แถม Glucerna SR กลูเซอนา เอสอาร์ กลิ่นวานิลลา 52.1g สำหรับผู้ป่วยเบาหวาน')";
$objQuery19 = mysqli_query($conn,$strSQL19);
...
$strSQL19 = "insert into so__submain
(ref_idd,sale_count,sale_countref,price_per_unit,price_per_unitref,sum_amount,discount_unit,product_id,product_code,warranty,status_sol,sale_remark)
values ('".$fetch21["ref_id"]."','1','1','0.00','0.00','0.00','0.00','5390','5390','','Approve','แถมผ้าเปียก ALLWELL 1 ห่อ เล็ก')";
$objQuery19 = mysqli_query($conn,$strSQL19);
...
(ref_idd,sale_count,sale_countref,price_per_unit,price_per_unitref,sum_amount,discount_unit,product_id,product_code,warranty,status_sol,sale_remark)
values ('".$fetch21["ref_id"]."','".$unit1."','".$unit1."','0.00','0.00','0.00','0.00','4170','4170','','Approve','แถมตลับใส่ยาเก่า')";
...
$strSQL15 = "insert into so__submain
(ref_idd,sale_count,sale_countref,price_per_unit,price_per_unitref,sum_amount,discount_unit,product_id,product_code,warranty,status_sol,sale_remark)
values ('".$fetch21["ref_id"]."','".$unit1."','".$unit1."','0.00','0.00','0.00','0.00','5494','5494','','Approve','แถมตลับใส่ยาเก่า')";
$objQuery15 = mysqli_query($conn,$strSQL15);
...
$strSQL6 = "Update   so__submain set ckk_free='1'   Where  ref_idd='".$main_id."' and sku_code='".$sku."'";
$objQuery6 = mysqli_query($conn,$strSQL6);		

	
} else if($sku=='4987067923809' or $sku=='4987067923908' or $sku=='4987067924004' or $sku=='4987067923106' or $sku=='4987067923205' or $sku=='4987067923304'  or $sku=='4987067140909' or $sku=='4987067141005' or $sku=='4987067141104'  or $sku=='4987067141807' or $sku=='4987067141906' or $sku=='4987067142002'  or $sku=='SUP-KOW-CFKNEM' or $sku=='SUP-KOW-CFKNEL' or $sku=='4987067923601'){

$strSQL6 = "Update   so__submain set ckk_free='2'   Where  ref_idd='".$main_id."' and sku_code='".$sku."'";
$objQuery6 = mysqli_query($conn,$strSQL6);		
...
/*$skus = array(
    "ASPR+GLS", "MIKO+GLS", "MNMX+GLS", "1L", "2L",
    "OSWHTH+GLS", "BSWHTH+GLS", "BLWH+GLS", "OLWHGK+GLS",
    "BP2005", "AW50TL", "AW100TL", "BP2005L", "BSX593",
    "gs-g42650", "gs-bsxt14", "gscl-G42650", "gscl-BSX593",
    "BSX593-L", "JPD-HA120", "BPM-BSX-532",
    "GCA50TL", "GCA100TL", "GCA50TLdup", "GCA100TLdup","GCA50Tdup","GCA25Tdup",
    "BSX593+GCA10", "BPM-URI-CARE", "GCA60", "GCA10", "GCA25T", "GCA50T", "AWL-GF-5"
);

// ตรวจสอบว่า $sku ไม่เป็นค่าว่างและอยู่ในรายการ $skus
if (!empty($sku) && in_array($sku, $skus)) {
	
$strSQL19 = "insert into so__submain
(ref_idd,sale_count,sale_countref,price_per_unit,price_per_unitref,sum_amount,discount_unit,product_id,product_code,warranty,status_sol,sale_remark)
values ('".$main_id."','1','1','0.00','0.00','0.00','0.00','4718','4718','','Approve','แถมพิเศษ โลชั่น ALLWELL ออร์แกนิค เดลี่ แคร์ 20 มล. เมื่อซื้อเครื่องวัดความดัน เครื่องวัดน้ำตาล และแผ่นเข็ม')";
$objQuery19 = mysqli_query($conn,$strSQL19);
...
$skuss = array(
    "AS30+GLS", "GLS (25) PRO", "GLS (50) PRO", "10T3L", "11T",
    "11T3L", "12T", "12T3L", "3T2L", "4T", "5T2L", "6T", "6T2L",
    "7T", "7T2L", "8T", "8T2L", "9T", "9T3L", "1T", "1T1L", "2T",
    "2T0.5L", "4T1L", "Glucosure Autocode 2", "Glucosure + HEM 7121",
    "Glucosure + HEM 7130", "Glucosure + ES-P370", "GLS+CMC", "GLS+SWK",
    "GLS+GWK", "GLS+FWK", "GLS100", "GLS150", "GLS200", "GLS25", "GLS50",
    "GLS252P", "GLS+SWC", "GLS+WK", "GLS+BP", "GLS+BPL", "2T0.5Ldup",
    "4T1Ldup"
);
...
$strSQL19 = "insert into so__submain
(ref_idd,sale_count,sale_countref,price_per_unit,price_per_unitref,sum_amount,discount_unit,product_id,product_code,warranty,status_sol,sale_remark)
values ('".$main_id."','1','1','0.00','0.00','0.00','0.00','4722','4722','','Approve','แถมพิเศษ โลชั่น ALLWELL ออร์แกนิค เดลี่ แคร์ 20 มล. เมื่อซื้อเครื่องวัดความดัน เครื่องวัดน้ำตาล และแผ่นเข็ม')";
$objQuery19 = mysqli_query($conn,$strSQL19);	
...
$strSQL19 = "insert into so__submain
(ref_idd,sale_count,sale_countref,price_per_unit,price_per_unitref,sum_amount,discount_unit,product_id,product_code,warranty,status_sol,sale_remark)
values ('".$main_id."','1','1','0.00','0.00','0.00','0.00','5638','5638','','Approve','Pro พิเศษ แถม Glucerna SR กลูเซอนา เอสอาร์ กลิ่นวานิลลา 52.1g สำหรับผู้ป่วยเบาหวาน')";
$objQuery19 = mysqli_query($conn,$strSQL19);
...
$strSQL19 = "insert into so__submain
(ref_idd,sale_count,sale_countref,price_per_unit,price_per_unitref,sum_amount,discount_unit,product_id,product_code,warranty,status_sol,sale_remark)
values ('".$main_id."','1','1','0.00','0.00','0.00','0.00','5637','5637','','Approve','Pro พิเศษ แถม Glucerna SR กลูเซอนา เอสอาร์ กลิ่นวานิลลา 52.1g สำหรับผู้ป่วยเบาหวาน')";
$objQuery19 = mysqli_query($conn,$strSQL19);
...
$sql02 = "SELECT ref_id FROM so__main WHERE sale_channel = '12' and register_date = '" . $register_date . "' and order_refer_code LIKE '%SD%'  and ckk_item='0'";
$result02 = mysqli_query($conn, $sql02);
$Num_Rows021 = mysqli_num_rows($result02);
	 
$sql05 = "SELECT ref_id FROM so__main WHERE sale_channel = '12' and register_date = '" . $register_date . "' and order_refer_code LIKE '%SH%'  and ckk_item='0'";
$result05 = mysqli_query($conn, $sql05);
$Num_Rows051 = mysqli_num_rows($result05);
	 
$kkkk =	$Num_Rows051+$Num_Rows021; 
	
$sql01 = "SELECT ref_id FROM so__main WHERE sale_channel = '12' and register_date = '" . $register_date . "' and order_refer_code LIKE '%TH24%'  and  ckk_item='0'";
$result01 = mysqli_query($conn, $sql01);
$Num_Rows011 = mysqli_num_rows($result01);	

$sql09 = "SELECT ref_id FROM so__main WHERE sale_channel = '12' and register_date = '" . $register_date . "' and order_refer_code LIKE '%TH%'  and order_refer_code NOT LIKE '%24%'  and  ckk_item='0'";
$result09 = mysqli_query($conn, $sql09);
$Num_Rows019 = mysqli_num_rows($result09);	
 
$sql20 = "SELECT ref_id FROM so__main WHERE sale_channel = '12' and register_date = '" . $register_date . "' and order_refer_code LIKE '%TH%'  and order_refer_code NOT LIKE '235%'  and  ckk_item='0'";
$result20 = mysqli_query($conn, $sql20);
$Num_Rows20 = mysqli_num_rows($result20);	
	 
	
$sql03 = "SELECT DISTINCT doc_no FROM so__main WHERE sale_channel = '12' and register_date = '" . $register_date . "' and doc_no LIKE '%E%' and ckk_item='0'";
...
$sql09 = "SELECT order_id FROM so__main WHERE doc_no='".$objResult8["doc_no"]."'";
$result09 = mysqli_query($conn, $sql09);
$objResult9 = mysqli_fetch_array($result09);	
$output .=  "คำสั่งซื้อเลขที่ : " .$objResult9["order_id"]. "  "; 
...
$sMessage = "
Shopee  นำเข้าเรียบร้อยค่ะ
Kerry Express : $kkkk 
SPX Express : $Num_Rows011
Flash : $Num_Rows019
Express Delivery : $Num_Rows20
...
$sql04 = "SELECT DISTINCT order_id FROM so__main WHERE sale_channel = '12' and register_date = '" . $register_date . "'  and ckk_item='0' and price_ckk='1'";
...
$sToken = "1B99f7Q2x3upAqGpl8ASgCufsAd4U32URULp8W8yNTJ"; 
$sMessage = "
Shopee รายการออเดอร์สินค้าราคาต่ำกว่ากำหนด
...
$sMessage = "
รบกวนตรวจสอบรายการออเดอร์สินค้าราคาต่ำกว่ากำหนด ร้านค้า Shopee
