...
$strSQL = "SELECT product_sn,service_order_no,work_type FROM tb_service_order WHERE service_order_id = '".$_GET["service_order_id"]."' ";
$objQuery = mysqli_query($servicenb,$strSQL) or die ("Error Query [".$strSQL."]");
$objResult = mysqli_fetch_array($objQuery);

$strSQL1 = "SELECT product_id FROM tb_products_in_stock WHERE product_sn = '".$objResult["product_sn"]."' ";
$objQuery1 = mysqli_query($servicenb,$strSQL1) or die ("Error Query [".$strSQL1."]");
$objResult1 = mysqli_fetch_array($objQuery1);

$strSQL2 = "SELECT register_id FROM tb_installation_data WHERE install_cus_id = '".$objResult["product_sn"]."' ";
$objQuery2 = mysqli_query($servicenb,$strSQL2) or die ("Error Query [".$strSQL2."]");
$objResult2 = mysqli_fetch_array($objQuery2);

$strSQL3 = "SELECT access_code,sol_name,unit_name FROM tb_product WHERE product_ID = '".$objResult1["product_id"]."' ";
$objQuery3 = mysqli_query($conn,$strSQL3) or die ("Error Query [".$strSQL3."]");
$objResult3 = mysqli_fetch_array($objQuery3);

$strSQL4 = "SELECT * FROM tb_customer WHERE customer_id = '".$objResult2["register_id"]."' ";
...
<div class="w3-white w3-container">
	...
	<form action="register_salechange1.php" method="post" name="frmMain" enctype="multipart/form-data" 
...
<?php if($_GET['service_order_no'] != ''){ ?><input type="hidden" name="service_order_no" id="service_order_no" value="<?php echo $_GET['service_order_no'];?>"><?php } // ดึงข้อมูลเลขที่เอกสารไปยังใบงานบริการ ?> 
	...
	<div class="w3-bar">
		...
		<input type="hidden" name="service_order_id" class="w3-input" value="<?php echo $_GET["service_order_id"]; ?>">	
	</div>
	...
	<div class="w3-half 1">
		...
		<div class="w3-bar w3-margin-bottom">
			<span>รหัสลูกค้า</span> <input type="text" name="customer_id" value="<?php echo $objResult4["customer_id"]; ?>" id="customer_id" class="w3-input" style="width:90%;"  OnChange="JavaScript:doCallAjax1('customer_id','customer','address','customer_typename');"  placeholder="Search ชื่อลูกค้า..." required>
			<input type ='hidden' name="h_customer"  id="h_customer" class="w3-input" >

		</div>
		<div class="w3-bar w3-margin-bottom">
			<span>ชื่อลูกค้า/รพ.</span> <input type="text" name="customer" value="<?php echo $objResult4["customer_name"]; ?>" id="customer" class="w3-input" style="width:90%;"  required>

		</div>
		<div class="w3-bar w3-margin-bottom">
			<span>ที่อยู่</span> <textarea name="address" id="address" class="w3-input" style="width:90%;" rows="2"><?php echo $objResult4["cus_address"]; ?> <?php echo $objResult4["cus_ampher"]; ?> <?php echo $objResult4["cus_province"]; ?> <?php echo $objResult4["cus_postcode"]; ?></textarea>
		</div>
...
<input name="slip5"  type="file">
		...
		<div class="w3-bar w3-margin-bottom w3-half">
			...
			<div class="w3-panel"><input type="radio"  name="objective" value="1" checked='cheched' id="objective" required> แลกเปลี่ยนสินค้า
				
					<input type="text" name="objective_des" class="w3-input" value="<?php echo $objResult["work_type"]; echo '-'; echo $objResult["service_order_no"]; ?>" placeholder="ใส่เลขที่ใบงานบริการ" style="width:90%;">
...
<?php
	...
	include ('detail_changengnb.php');
	}else{
		include ('detail_changehosnb.php');
	}
	?>
...
<script>
// ดึงค่า service_order_no จาก URL parameters
let urlParams = new URLSearchParams(window.location.search);
let hospital_name = urlParams.get('hospital_name');
document.getElementById('customer').value = hospital_name;
</script>