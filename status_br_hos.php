	...
	<table id="myTable" class="w3-table w3-striped w3-bordered" border=1>
		...
		<tbody>
			<?php
				...
				$sol = "select * from hos__br where sale_code='".$_SESSION["emid"]."'";
				if($start_date&&$end_date!="0000-00-00"){
				$sol .= "and date between '".$start_date."' and '".$end_date."'";
				}
				if($keyword!=""){
				$sol .= "and customer like '%".$keyword."%'";
				}				
				...
				<tr>
					 ...
					 <td style="text-align:left;"><?php $id=$result['ref_id'];
							   $pd=mysqli_query($conn,"select * from hos__subbr left join tb_product on (hos__subbr.product_id=tb_product.product_ID) where ref_id='$ref_id'");
...
<div class="w3-bar w3-margin-top">
  <a href="status_br_hos.php?page=1" class="w3-button w3-light-grey">&laquo;</a>
  <?php for($i=1;$i<=$total_page;$i++){ ?>
  <a href="status_br_hos.php?page=<?php echo $i; ?>" class="w3-button w3-light-grey"><?php echo $i; ?></a>
  <?php } ?>
  <a href="status_br_hos.php?page=<?php echo $total_page;?>" class="w3-button w3-light-grey" aria-label="Next">
