...
<div class="w3-container w3-bar w3-light-gray w3-margin-bottom"><h4>Status Admin </h4></div>
...
<th width="5%" style="font-size: 12px;">VIP</th>
<th width="5%" style="font-size: 12px;">สถานะการอนุมัติ</th>
<th width="5%" style="font-size: 12px;">เวลาอนุมัติ</th>
<th width="5%" style="font-size: 12px;">แก้ไข</th>
	
<?php if  ($_SESSION['name']!="บรรเทิง"){ ?>	
<th width="5%" style="font-size: 12px;">สร้าง</th>
<th width="2%" style="font-size: 12px;">สร้างใบลดหนี้</th>
<th width="2%" style="font-size: 12px;">ใบกำกับภาษี</th> 
<th width="2%" style="font-size: 12px;">สร้างใบรับสินค้า</th> 
<!--th width="2%" style="font-size: 12px;">การส่งบิล</th--> 
<th width="2%" style="font-size: 12px;">คืนสินค้า</th> 
...
<td style="font-size: 12px;"><?php echo $objResult["order_id"];?>
	<?php if  ($_SESSION['name']=="อัจฉรา" or $_SESSION['name']=="นงลักษณ์"  or $_SESSION['name']=="สุดารัตน์"){ ?><br>
...
<?php
	...
	<td style="font-size: 12px;"><?php if($objResult["approve_date"]=='0000-00-00 00:00:00'){ }else{ echo $objResult["approve_date"]; } ?></td>
...
$sql5 = "SELECT ref_credit   FROM tb_credit_note where ref_id ='".$objResult["ref_id"]."' ";
$qry5 = mysqli_query($conn,$sql5) or die(mysqli_error());
$rs5 = mysqli_fetch_assoc($qry5);

if($rs5["ref_credit"]){
?>	
<a href="report_credit_adm.php?ref_credit=<?php echo $rs5["ref_credit"];?>"><img src="img/print_icon-2.png" width="23" height="23" border="0" /></a>
<?php }else{ ?>	
<a href=javascript:if(confirm('!!!ต้องการสร้างใบสั่งลดหนี้ใช่หรือไม่')==true){window.location='register_creditsol.php?ref_id=<?php echo $objResult["ref_id"];?>';}><img src="img/sticker.png" width="23" height="23" border="0" /></a>
<?php } ?>	
...
<a href=javascript:if(confirm('!!!ต้องการสร้างใบรับสินค้าใช่หรือไม่')==true){window.location='register_receivepro_aw.php?ref_id=<?php echo $objResult["ref_id"];?>';}><img src="img/preview.jpg" width="23" height="23" border="0" /></a>

</td>	
	
	<!--td>	
	<?php if($objResult["cus_sb"]=='1'){ ?>
<img src="img/success.gif" width="23" height="23" border="0" />
		<?php } ?>
</td-->
	<td>
	
<a href=javascript:if(confirm('!!!ต้องการคืนสินค้าใช่หรือไม่')==true){window.location='register_clear_ecom.php?ref_id=<?php echo $objResult["ref_id"];?>';}><img src="img/sticker.png" width="23" height="23" border="0" /></a>		
</td>
