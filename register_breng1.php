...
(ref_idd_br,count,countref,price,amount,sale_remark,product_id,product_code,br_periodd,warranty)
values ('".$ref_id_br."','".$sale_count6."','".$sale_count6."','".$product_price6."','".$sum_amount6."','".$sale_remarkk6."','".$product_id6."','".$product_id6."','".$br_period6."','".$warranty6."','".$warranty6."')";
...
// อ้างอิงข้อมูลไปยังใบงานบริการ
if($service_order_no != ''){

	$ensql = "UPDATE tb_service_order SET ls_no = '".$ref_id_br."' WHERE service_order_no = '".$service_order_no."' ";
	if($company == 1){ // AWL
		$qensql1 = mysqli_query($service,$ensql);
	} else if($company == 2){ // NBM
		$qensql2 = mysqli_query($servicenb,$ensql);
	}
...
echo "alert('บันทึกข้อมูลของท่านเรียบร้อยแล้ว');window.location='register_breng_edit.php?ref_id_br=$ref_id_br';";
echo "</script>";
  } else {
   echo "Cannot";
  }
