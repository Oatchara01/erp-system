...
<div class="w3-panel w3-light-grey"><h3>แบบสอบถามที่นอนโฟม ขนส่งนอก</h3></div>	
...
$strSQL = "SELECT ref_id,select_type_doc,register_date,doc_no,doc_release_date,delivery_contact,approve_complete,cancel_ckk,order_refer_code,order_refer_code1,employee_name,sale_channel  FROM so__main  where allwell_ckk = '1' and sale_channel ='4' and approve_complete='Approve' and doc_no NOT LIKE '%BRN%' and doc_no LIKE '%IV%' and delivery='1'";

if($start_date !=""){ 
    $strSQL .= ' AND register_date >= "'.$start_date.'"'; 
}
	else{
    $strSQL .= ' AND register_date >= "2022-04-01"'; 
...
<div class="w3-container">
	<table border="1" width="100%" class="w3-table">
		<thead class="w3-gray">
			...
			<td width="2%">แบบสอบถามที่นอนโฟม ขนส่งนอก</td >
...
$strSQL1 = "SELECT sol_name,research_ckk FROM (so__submain LEFT JOIN tb_product ON so__submain.product_ID=tb_product.product_id) WHERE ref_idd = '".$objResult["ref_id"]."' ";
...
<a href=javascript:if(confirm('!!!ต้องการทำแบบสอบถามสินค้าประเภทDใช่หรือไม่')==true){window.location='register_commentb.php?ref_id=<?php echo $objResult["ref_id"];?>';}><img src="img/receipt_product.png" width="23" height="23" border="0" /></a>
...
<?php /* <div class="w3-panel">    <strong>พบทั้งหมด</strong>
      <?= $Num_Rows;?>
