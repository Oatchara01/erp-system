...
$strSQL = "SELECT *  FROM tb_credit_note  where  status_doc = 'Request' and send_dm = '1'";
...
?>
 ...
 echo DateThai($objResult["date_credit"]);
	...
	?><br />
				...
				<td><div align="left">
					<?php
						$strSQL3 ="SELECT sol_name FROM (tb_subcredit LEFT JOIN tb_product ON tb_subcredit.product_ID=tb_product.product_id) WHERE ref_creditt = '".$objResult["ref_credit"]."'";
						$objQuery3 = mysqli_query($conn,$strSQL3) or die ("Error Query [".$strSQL3."]");
						$Num_Rows3 = mysqli_num_rows($objQuery3);

						while($objResult3 = mysqli_fetch_array($objQuery3)) { ?>
							<?php
 	echo $objResult3["sol_name"]; 
	?><br />
						<?php } ?>
				</div></td>
<?php
$sql = "SELECT SUM(sum_amount) AS sum_amount  FROM tb_subcredit where  ref_creditt = '".$objResult["ref_credit"]."'";
$qry = mysqli_query($conn,$sql) or die(mysqli_error());
$rs = mysqli_fetch_assoc($qry);		
?>	
<td><div align="right"><?php echo number_format($rs["sum_amount"],2).""; ?></div></td>
				<td ><div align="left"><?php echo $objResult["customer_name"];?></div></td>
								

					<td>

			<a href="register_credit_dmapprove.php?ref_credit=<?php echo $objResult["ref_credit"];?>&start_date=<?php echo $_GET["start_date"];  ?>&end_date=<?php echo $_GET["end_date"];?>"><img src="img/sticker.png" width="23" height="23" border="0" /></a>
