...
/*
//ปิดอยู่แล้ว
$strSQL = "SELECT order_id, cs_remark, stock_remark FROM so__main WHERE sale_channel = '12' AND ckk_item = '0' and cs_remark LIKE '%SPX Express%' and stock_remark != 'pickup'";
$objQuery = mysqli_query($conn, $strSQL) or die ("Error Query [".$strSQL."]");
...
$strSQL = "SELECT order_id, cs_remark, stock_remark FROM so__main WHERE sale_channel = '12'  AND ckk_item='0' and cs_remark NOT LIKE '%Instant%'";
$objQuery = mysqli_query($conn, $strSQL) or die ("Error Query [".$strSQL."]");
...
while ($objResult = mysqli_fetch_array($objQuery)) {
   ...
   if ($cs_remark == 'SPX Express' OR $cs_remark=='Express Delivery - ส่งด่วน' OR $cs_remark=='Express Delivery (SHP Food)') {  
