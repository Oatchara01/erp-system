...
<div class="w3-panel w3-light-grey"><h3>Status Search Product</h3></div>

<div class="w3-bar w3-quarter">
วันที่ : <input name="start_date" class="w3-input" style="width:90%;" type="date" id="start_date" value=<?php echo $start_date = isset($_GET['start_date']) ? $_GET['start_date'] : ''; ?> ></div>
<div class="w3-bar w3-quarter">
ถึง :<input name="end_date" class="w3-input" style="width:90%;" type="date" id="end_date" value=<?php echo $end_date = isset($_GET['end_date']) ? $_GET['end_date'] : ''; ?> ></div>
<div class="w3-bar w3-quarter">
	
ค้นหา : <input name="product_code" class="w3-input" style="width:90%;" type="text" id="product_code" value="<?php echo $Keyword = isset($_GET['product_code']) ? $_GET['product_code'] : '';?>">
	<input type='hidden' name = "h_product_code"  id = "h_product_code"  class="w3-input" readonly></div>
<div class="w3-bar w3-quarter w3-padding-xsmall">
<input type="submit" class="w3-button w3-teal" value="Search"></div>
</div>
</form>
<?php	
	$Keyword = isset($_GET['product_code']) ? $_GET['product_code'] : '';
...
$strSQL = "SELECT ref_id,select_type_doc,register_date,doc_no,doc_release_date,access_name,delivery_contact,approve_complete  FROM ((so__submain LEFT JOIN so__main ON so__submain.ref_idd=so__main.ref_id)LEFT JOIN tb_product ON so__submain.product_id=tb_product.product_ID)  where 1 ";
...
if($Keyword !=""){ //แสดงว่ามีค่า end_date ส่งมา หรือมีการค้นหา ก็ใหเต่อ String query
	$strSQL .= ' AND access_name  LIKE "%'.$Keyword.'%"'; 
