...
<form  method="POST" action = "supsmp_approve1.php" name="frmMain" enctype="multipart/form-data"  onSubmit="JavaScript:return fncSubmit();">
	
<div class="w3-white w3-container">
	<div class="w3-panel w3-light-grey">
	<div class="w3-half"><h3>ใบเบิกสินค้าเพื่อสนับสนุนการขาย</h3>	
...
<?php
if($_SESSION['name']=='รุจิรา' or $_SESSION['name']=='ลักษณาวรรณ'){	
if($ffirst["sale_code"]=='SOL1' or $ffirst["sale_code"]=='SOL2' or $ffirst["sale_code"]=='SOL99' or $ffirst["sale_code"]=='SOL3'  or $ffirst["sale_code"]=='SOL4'  or $ffirst["sale_code"]=='SOL5' or $ffirst["sale_code"]=='SOL91' or $ffirst["sale_code"]=='SOL92' or $ffirst["sale_code"]=='SOL93' or $ffirst["sale_code"]=='SOL94' or $ffirst["sale_code"]=='MK'){
?>
<input type="button" name ="Submit" value="Approve" class = "w3-button w3-green button4" onClick="this.form.action='sample_approve1.php'; submit()">

<?php }else{ ?>
<input type="button" name ="Submit" value="Approve" class = "w3-button w3-green button4" onClick="this.form.action='sample_approve.php'; submit()">
<?php 
		   }
}else{	
		?>	
...
<?php } ?>			
<!--input type="button" name ="Submit" value="Approve" class = "w3-button w3-green button4" onClick="this.form.action='sample_approve.php'; submit()"-->
<input type="button" name ="button" value="Rejected" class = "w3-button w3-red button4" onClick="this.form.action='sample_rejected.php'; submit()">
		</div></div>
...
<input name="up_img3"    type="file"><a href="smp_up/<?php echo $ffirst['up_img2']; ?>" target="_blank"><?php echo $ffirst['up_img2']; ?></a>
</div>		
...
<div class="w3-bar w3-margin-bottom">	
	ID ลูกค้า
<input type='text' name = "bill_id" value = "<?php echo $ffirst['bill_id']; ?>"  id = "bill_id" style="width:90%;" class="w3-input" placeholder="Search ชื่อลูกค้า..."   OnChange="JavaScript:doCallAjax1('bill_id','customer_name','customer_tel','address_name');" required> 
...
<div class="w3-bar w3-margin-bottom">
			หมายเลขคำสั่งซื้อ 
			<input type="text" name="order_id" id="order_id" value = "<?php echo $ffirst['order_id']; ?>" class="w3-input" style="width:90%;"  required>
</div>	
<div class="w3-bar w3-margin-bottom">
			Comment Sale :&nbsp;
...
$strSQL5 = "SELECT * FROM tb_team_ss3 where sale_code IN ('S32','S31')  ORDER BY sale_code ASC";
//echo $strSQL5;
//exit();
$objQuery5 = mysqli_query($com,$strSQL5);
while($objResuut5 = mysqli_fetch_array($objQuery5))
...
<td  style="width:8%;">
<textarea  name = "product_name[<?php echo $objResult1["subsmp_id"]; ?>]"  id = "product_name[<?php echo $objResult1["subsmp_id"]; ?>]"   rows="2" class="w3-input" readonly><?php echo $objResult1["access_name"]; ?></textarea>
</td>
<td style="width:5%;">
...
<td  style="width:8%;">
<textarea  name = "sale_remarkk[<?php echo $objResult1["subsmp_id"]; ?>]"  id = "sale_remarkk[<?php echo $objResult1["subsmp_id"]; ?>]"   rows="2" class="w3-input" readonly><?php echo $objResult1["sale_remark"]; ?></textarea>
</td>
...
<?php
	 if($ffirst["send_stock"]=='0'){ 
	 if($ffirst["send_dm"]=='0'){ 
	
	if($_SESSION["department"]=='วิศวกรรม'){
		if ($ffirst['type_company']=='1'){
		include ('detail_engsmp1.php');
		}else if ($ffirst['type_company']=='2'){
		include ('detail_engsmpnb1.php');	
		}
	}else{	
			if ($ffirst['type_company']=='1'){
	include ('detail_salesmp1.php');
			}else 	if ($ffirst['type_company']=='2'){
	include ('detail_salesmpnb1.php');			
			}
		}
	 }
	 }
...
ที่อยู่ส่งสินค้า :</p>
<input type='text' value="<?php echo $fetch1["address_1"]; ?>" class="button4" name="address_1" style="width:50%" > </p>            
...
<div class="w3-bar w3-center">
			<input type="submit" name="submit1" class="w3-button w3-teal be-border" value="บันทึกข้อมูล">
	</div><br>
</div></form>
