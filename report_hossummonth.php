...
$strSQL = "SELECT *  FROM  hos__so  where  iv_date LIKE '%$start_date%' AND sale_code ='".$sale_code."' and status_doc = 'Approve' and ic_ckk ='0'";
...
$strSQL = "SELECT *  FROM  tb_credit_note  where  date_credit LIKE '%$start_date%' AND sale_code ='".$sale_code."' and status_doc = 'Approve'";
$objQuery = mysqli_query($conn,$strSQL) or die ("Error Query [".$strSQL."]");
$Num_Rows = mysqli_num_rows($objQuery);
...
<td><a href="register_credit_adm.php?ref_credit=<?php echo $objResult["ref_credit"];?>"  target="_blank"><?php echo $objResult["credit_no"];?></a>
				...
				<td><div align="left">
					<?php
$strSQL2 = "SELECT sol_name FROM (tb_subcredit LEFT JOIN tb_product ON tb_subcredit.product_ID=tb_product.product_id) WHERE ref_creditt = '".$objResult["ref_credit"]."' ";
$objQuery2 = mysqli_query($conn,$strSQL2) or die ("Error Query [".$strSQL2."]");
$Num_Rows2 = mysqli_num_rows($objQuery2);
while($objResult2 = mysqli_fetch_array($objQuery2)) { ?>
<?php echo $objResult2["sol_name"];	?><br>
...
$strSQL14 = "SELECT SUM(sum_amount) AS sum_amount FROM (tb_credit_note LEFT JOIN tb_subcredit ON tb_subcredit.ref_creditt=tb_credit_note.ref_credit)  WHERE date_credit LIKE '%$start_date%' AND sale_code ='".$sale_code."' and status_doc = 'Approve'";
$objQuery14 = mysqli_query($conn,$strSQL14) or die ("Error Query [".$strSQL14."]");
$objResult14 = mysqli_fetch_array($objQuery14); 
	
	
	
$strSQL4 = "SELECT SUM(amount) AS amount FROM (hos__so LEFT JOIN hos__subso ON hos__subso.ref_idd=hos__so.ref_id)  WHERE  iv_date LIKE '%$start_date%' AND sale_code ='".$sale_code."' and status_doc = 'Approve'  and ic_ckk ='0'";
$objQuery4 = mysqli_query($conn,$strSQL4) or die ("Error Query [".$strSQL4."]");
$objResult4 = mysqli_fetch_array($objQuery4); 
	
	echo number_format(($objResult4["amount"]-$objResult14["sum_amount"]),2);	?>
				</div></td>				
