...
$sql = "select * from $table_db  where close_ckk = '0' and customer_id = '".$customer_id."' and customer_id !='799' and customer_id !='678' and  locate('$product_code_search', $find_field) > 0 order by locate('$product_code_search', $find_field), $find_field limit $pagesize";
...
$strSQL77 = "SELECT code_zone FROM tb_user_zone WHERE em_id = '".$em_id."' ";
...
while ($objResult77 = mysqli_fetch_array($objQuery77)) {
    ...
    $strSQL71 = "SELECT id_customer FROM tb_selected_sales WHERE sale_code = '".$zone_code."' ";
    ...
    while ($objResult71 = mysqli_fetch_array($objQuery71)) {
        ...
        $sql = "SELECT customer_name,bill_name,customer_id FROM $table_db  
                WHERE close_ckk = '0' 
...
$table_db=" tb_customer"; // ตารางที่ต้องการค้นหา
$find_field="customer_name"; // ฟิลที่ต้องการค้นหา 
$sql = "select * from $table_db  where close_ckk = '0' and customer_id !='799' and customer_id !='678' and  locate('$product_code_search', $find_field) > 0 order by locate('$product_code_search', $find_field), $find_field limit $pagesize";
