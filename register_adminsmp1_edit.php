...
<!--a href="sample_sendstock1.php?ref_idsmp=<?php echo $_GET["ref_idsmp"]; ?>" target="_blank" class="w3-button w3-green w3-right"><font color="black">ส่งใบเบิกให้ Stock</font></a-->&nbsp;&nbsp;

<?php if($_SESSION['name']=='พัชร์ชนัญ'){	?>
<a href="sample_senddm1.php?ref_idsmp=<?php echo $_GET["ref_idsmp"]; ?>" target="_blank" class="w3-button w3-blue w3-right"><font color="black">ส่งให้พี่โจ้ Approve</font></a>
	<?php } ?>
	<?php if($ffirst['send_sup']=='0' and $ffirst['status_sup'] !='Approve'){	?>
<a href="send_allsample_approve.php?ref_idsmp=<?php echo $_GET["ref_idsmp"];?>&sale_code=<?php echo $_SESSION["code"]; ?>" target="_blank" class="w3-button w3-grey w3-right"><font color="red">ส่งใบเบิกให้ Sup อนุมัติ</font></a>&nbsp;&nbsp;
	<?php } ?>
		...
		<form action="register_adminsmp_edit1.php" method="post" name="frmMain" enctype="multipart/form-data">
<div class="w3-bar">
...
<input type='hidden' name = "sale_countref[]<?php echo $objResult1["subsmp_id"]; ?>" id = "sale_countref[]<?php echo $objResult1["subsmp_id"]; ?>" value = "<?php echo $objResult1["sale_countref"]; ?>"  />
...
<td style="width:5%;">
<input type='text' name = "waranty[]<?php echo $objResult1["subsmp_id"]; ?>"  id = "waranty[]<?php echo $objResult1["subsmp_id"]; ?>" value = "<?php echo $objResult1["waranty"]; ?>" class="w3-input" readonly/>
</td>	
	<td  style="width:8%;">
...
<td style="width:5%;"><input type='text' name = "clear_br[]<?php echo $objResult1["id"];?>" value="<?php echo $objResult1["clear_br"];?>" id = "clear_br[]<?php echo $objResult1["id"];?>"  class="w3-input"   OnKeyPress="return chkNumber(this)"/>
	<input type='text' name = "br_no[]<?php echo $objResult1["id"];?>" value="<?php echo $objResult1["br_no"];?>" id = "br_no[]<?php echo $objResult1["id"];?>" placeholder="เลขที่ใบยืม"  class="w3-input"  />
	</td>
<td style="width:2%;"><a href="delete_prosmp.php?subsmp_id=<?php echo $objResult1["subsmp_id"];?>&code=<?php echo $_SESSION["type_login"]; ?>&ref_idsmp=<?php echo $objResult1["reff_idsmp"];?>"><img src="img/false.png" width="16" height="16" border="0" /></a></td>
