...
<body>
	...
	$strSQL91 = "SELECT clear_ivno FROM hos__subso WHERE ref_idd = '" . $ref_id . "' AND clear_ivno !=''";
	$objQuery91 = mysqli_query($conn, $strSQL91) or die("Error Query [" . $strSQL91 . "]");

	while ($objResult91 = mysqli_fetch_array($objQuery91)) {
		$clearIvnos[] = $objResult91["clear_ivno"];
	}

	$uniqueIvnos = array_unique($clearIvnos);
	$ivno_ref = implode('/', $uniqueIvnos); // ใช้ / คั่นค่าทั้งหมด

	$save52 = "UPDATE hos__so SET brnp_no = '$ivno_ref' WHERE ref_id = '" . $ref_id . "'";
	$qsave52 = mysqli_query($conn, $save52);
...
";
	...
	<form action='register_suphos_edit1.php' method="post" name="frmMain" enctype="multipart/form-data"
		...
		<script>
			(function() {
				...
				var CREDIT_PAYMENT_CODES = ['36', '38', '39', '40', '41', '42', '44', '45'];

				// ====== modal open/close ======
				window.closeCreditAlert = function() {
					var modal = document.getElementById('creditAlertModal');
				...
				function calcGrandTotal() {
					...
					for (var i = 6; i <= 20; i++) {
						var oldEl = document.getElementById('sum_amount' + i);
						if (oldEl) total += parseNum(oldEl.value);
					}

					var totalEl = document.getElementById('sum_amount_total');
					if (totalEl) totalEl.value = format2(total);

					return total;
				}

				// ====== เช็คว่าการชำระเงินอยู่ในกลุ่มเครดิตที่ต้องตรวจหรือไม่ ======
				function isCreditPayment() {
					var paymentEl = document.getElementById('payment');
					if (!paymentEl) return false;

					var paymentValue = String(paymentEl.value).trim();
					return CREDIT_PAYMENT_CODES.includes(paymentValue);
				}

				// ====== ดักปุ่มส่งอนุมัติ ======
				function bindApproveButton() {
					var btn = document.getElementById('btnSendApprove');
					if (!btn) return;
				...
				document.addEventListener('input', function(e) {
					...
					if (
						...
						t.classList.contains('js-price') ||
						t.classList.contains('js-disc')
					) {
    ...
    checkCredit();
		...
		<div class="w3-white w3-container w3-padding-large"><!-- main div -->
				...
				<div class="w3-half">
					...
					?>
							<?php if ($rs["adm_ckk"] == '1') {
								...
								<!--a href="send_soadmin.php?ref_id=<?php echo $rs["ref_id"]; ?>&sale_code=<?php echo $_SESSION["code"]; ?>" target="_blank" class="w3-button w3-grey w3-right"><font color="red">ส่งใบสั่งขายให้ Admin</font></a-->

								<a id="btnSendApprove"
									href="send_soadmin.php?ref_id=<?php echo $rs["ref_id"]; ?>&sale_code=<?php echo $_SESSION["code"]; ?>"
									target="_blank"
									class="w3-button w3-yellow w3-right">
									<font color="red">ส่งใบสั่งขายให้ Admin</font>
								</a>


								<!--input type="button" name ="Submit"  value="ส่งใบสั่งขายให้ Admin" class="w3-button w3-red w3-right" onClick="this.form.action='send_soadmin.php'; submit()" -->
					...
					<a href="report_salehosptl2.php?ref_id=<?php echo $rs["ref_id"]; ?>" target="_blank" class="w3-button w3-grey w3-right">
						<font color="Purple">แบบฟอร์มใบสั่งขาย</font>
					</a><br />
			...
			<div class="w3-bar">
				...
				&nbsp;&nbsp;&nbsp;&nbsp;
				<?php if ($rs["et_ckk"] == '1') { ?>
					<input type="checkbox" name="et_ckk" checked='cheched' id="et_ckk" value="1">
					<font color='blue'>ขอบิล E-Tax</font>
				<?php } else { ?>
			...
			<?php if ($rs["have_order"] == '1') { ?>
				...
				<input type="checkbox" name="have_order" id="have_order" checked='checked' value="1"> ออเดอร์ฝาก
			...
			<?php if ($rs["que_ckk"] == '1') { ?>
				...
				<input type="checkbox" name="que_ckk" id="que_ckk" value="1" checked='checked'>
				<font color='red'> งานด่วน </font>
			...
			<select id="payment" name="payment" class="button4" style="width:30%">
...
<textarea name="sale_comment"  class="button4" style="width:30%" id="sale_comment"  rows="2"><?php echo $rs["sale_comment"];?></textarea>*/ ?></p>
			...
			กำหนดส่งตามสัญญา :
			<input name="delivery_contract" id="delivery_contract" type='date' value="<?php echo $rs["delivery_contract"]; ?>" class="button4" style="width:30%" placeholder="กรุณาระบุเป็นวันที่เท่านั้น !!!"> </p>
			...
			<br>
			<fieldset>
				<legend><b>หมายเหตุแจ้งแผนกที่เกี่ยวข้อง</b></legend>
				<br>
			...
			<div class="w3-bar w3-light-grey w3-border">
				<a class="w3-bar-item w3-button" onclick="openCity1('pd')">
					<font color="#404040"><b>รายการสินค้า</b></font>
				</a>
				<a class="w3-bar-item w3-button" onclick="openCity1('cs')">
					<font color="#404040"><b>รายละเอียดการจัดส่ง</b></font>
				</a>
				<a class="w3-bar-item w3-button" onclick="openCity1('cs1')">
					<font color="#404040"><b>การจัดส่ง(เพิ่มเติม)</b></font>
				</a>
					...
					<tr>
						...
						<td style="width:15%;">
							<textarea
								...
								class="<?= h($nameClass) ?>"
								readonly><?= h($row['sol_name'] ?? $row['bom_name'] ?? '') ?></textarea>
						</td>
			...
			<div id="cs" class="w3-container city1" style="display:none">
				...
				?>
					<fieldset>
						<legend><input type="checkbox" name="more" id="more" value="1"> <b>รายละเอียดการจัดส่ง</b></legend>
				...
				?>
					<fieldset>
						<legend><input type="checkbox" name="more" id="more" value="1" checked="checked"> <b>รายละเอียดการจัดส่ง</b></legend>
			...
			<div id="cs1" class="w3-container city1" style="display:none">
				...
				<fieldset>
					<legend><b>รายละเอียดการจัดส่ง (เพิ่มเติม1)</b></legend>
				...
				<fieldset>
					<legend><b>รายละเอียดการจัดส่ง (เพิ่มเติม2)</b></legend>
				...
				<fieldset>
					<legend><b>รายละเอียดการจัดส่ง (เพิ่มเติม3)</b></legend>
				...
				<fieldset>
					<legend><b>รายละเอียดการจัดส่ง (เพิ่มเติม4)</b></legend>
				...
				<fieldset>
					<legend><b>รายละเอียดการจัดส่ง (เพิ่มเติม5)</b></legend>
				...
				<fieldset>
					<legend><b>รายละเอียดการจัดส่ง (เพิ่มเติม6)</b></legend>
				...
				<fieldset>
					<legend><b>รายละเอียดการจัดส่ง (เพิ่มเติม7)</b></legend>
				...
				<fieldset>
					<legend><b>รายละเอียดการจัดส่ง (เพิ่มเติม8)</b></legend>
				...
				<fieldset>
					<legend><b>รายละเอียดการจัดส่ง (เพิ่มเติม9)</b></legend>
	...
	<div id="creditAlertModal" style="display:none;position:fixed;inset:0;z-index:99999;background:rgba(0,0,0,.5);">
		<div style="position:relative;margin:10vh auto;width:min(680px,92vw);background:#fff;border-radius:14px;box-shadow:0 10px 30px rgba(0,0,0,.25);overflow:hidden;">
			<button type="button" onclick="window.location.href='status_suphos.php';" style="position:absolute;right:14px;top:10px;border:0;background:transparent;font-size:40px;cursor:pointer;">×</button>
			...
			<div style="padding:28px 18px 10px;text-align:center;">
				...
				<button type="button"
					onclick="window.location.href='status_suphos.php';"
