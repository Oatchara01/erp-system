...
<div class="w3-container w3-bar w3-light-gray w3-margin-bottom"><h4>รายการออเดอร์เครื่องวัดน้ำตาล G-426</h4></div>
...
$strSQL = "SELECT *  FROM tb__glu426  where sn !='' ";
if($Keyword !=""){ 
...
<td style="font-size: 12px;"><div align="left"><?php
if (!empty($objResult["sale_chan"]) && isset($objResult2["salechannel_nameshort"])) {
    echo htmlspecialchars($objResult2["salechannel_nameshort"], ENT_QUOTES, 'UTF-8');
}
?>
</div></td>
...
<a href=javascript:if(confirm('!!!ต้องการเปิดเอกสารSMPแลกเปลี่ยนสินค้าใช่หรือไม่')==true){window.location='register_chang426h.php?ref_id=<?php echo $objResult["ref_id"];?>&id=<?php echo $objResult["id"];?>';}><img src="img/receipt_product.png" width="23" height="23" border="0" /></a>	
	<?php }else if($go=='SM'){ ?>
		
<a href=javascript:if(confirm('!!!ต้องการเปิดเอกสารSMPแลกเปลี่ยนสินค้าใช่หรือไม่')==true){window.location='register_chang426smp.php?ref_id=<?php echo $objResult["ref_id"];?>&id=<?php echo $objResult["id"];?>';}><img src="img/receipt_product.png" width="23" height="23" border="0" /></a>			
		<?php }else{ ?>
<a href=javascript:if(confirm('!!!ต้องการเปิดเอกสารSMPแลกเปลี่ยนสินค้าใช่หรือไม่')==true){window.location='register_chang426.php?ref_id=<?php echo $objResult["ref_id"];?>&id=<?php echo $objResult["id"];?>';}><img src="img/receipt_product.png" width="23" height="23" border="0" /></a>	
...
<td style="font-size: 12px;">
		<?php if (isset($rs12["img_upsn"]) && $rs12["img_upsn"] != '') { ?>
    <a href="smp_up/<?php echo htmlspecialchars($rs12["img_upsn"], ENT_QUOTES, 'UTF-8'); ?>" target="_blank">
        <font color='blue'>Click</font>
    </a>
<?php } ?>
...
?>
 ...
 <div class="w3-panel"  style="font-size: 12px;">    <strong>พบทั้งหมด</strong>
	...
	if($Prev_Page)
	{
		echo " <a href='$_SERVER[SCRIPT_NAME]?Page=$Prev_Page&Keyword=$Keyword&Keyword1=$Keyword1&Keyword2=$Keyword2&sale_channel=$sale_channel'><font color='black'><< Back</font></a> ";
	}

	for($i=1; $i<=$Num_Pages; $i++){
		if($i != $Page)
		{
			echo "[ <a href='$_SERVER[SCRIPT_NAME]?Page=$i&Keyword=$Keyword&Keyword1=$Keyword1&Keyword2=$Keyword2&sale_channel=$sale_channel'><font color='black'>$i</font></a> ]";
	...
	if($Page!=$Num_Pages)
	{
		echo " <a href ='$_SERVER[SCRIPT_NAME]?Page=$Next_Page&Keyword=$Keyword&Keyword1=$Keyword1&Keyword2=$Keyword2&sale_channel=$sale_channel'><font color='black'>Next>></font></a> ";
	}
