...
<?php if($_SESSION['user_type']=='Engineer'){ ?>	
	<div class="w3-bar w3-quarter">เขตการขาย
<select name="sale_code1" id="sale_code1" style="width:280px" class="w3-input" >
...
{
if($_GET['sale_code1'] == $objResuut5["sale_code"])
{
$sel = "selected";
}
else
...
<?php	
	...
	$sale_code1 = isset($_GET['sale_code1']) ? $_GET['sale_code1'] : '';
...
$strSQL = "SELECT *  FROM hos__br  where sale_code LIKE '%EN%' ";
}else{
$strSQL = "SELECT *  FROM hos__br  where sale_code = '".$code."' ";	
}
...
if($sale_code1 !=""){ 
    $strSQL .= ' AND sale_code = "'.$sale_code1.'"'; 
}
...
?>
 ...
 echo DateThai($objResult["date_br"]);
				...
				<td><div align="left">
					<?php
						$strSQL8 = "SELECT sn FROM hos__subbr WHERE ref_idd_br = '".$objResult["ref_id_br"]."' ";
	...
	?><br>
				...
				<a href="register_breng_edit.php?ref_id_br=<?php echo $objResult["ref_id_br"];?>&start_date=<?php echo $_GET["start_date"];  ?>&end_date=<?php echo $_GET["end_date"];?>"><img src="img/edit-icon.png" width="23" height="23" border="0" /></a>
				<?php }else{ ?>
				<a href="register_brhos_edit.php?ref_id_br=<?php echo $objResult["ref_id_br"];?>&start_date=<?php echo $_GET["start_date"];  ?>&end_date=<?php echo $_GET["end_date"];?>"><img src="img/edit-icon.png" width="23" height="23" border="0" /></a>	
					<?php } ?>
				</td>
				<td>
...
<td>
<?php if($user_type=='Engineer'){ ?>	
<a href=javascript:if(confirm('!!!ต้องการเพิ่มเอกสารใหม่โดยCopyเอกสารเดิมใช่หรือไม่')==true){window.location='register_breng_createnew.php?ref_id_br=<?php echo $objResult["ref_id_br"];?>&start_date=<?php echo $_GET["start_date"];  ?>&end_date=<?php echo $_GET["end_date"];?>';}><img src="img/sticker.png" width="23" height="23" border="0" /></a>		
	<?php }else{ ?>
<a href=javascript:if(confirm('!!!ต้องการเพิ่มเอกสารใหม่โดยCopyเอกสารเดิมใช่หรือไม่')==true){window.location='register_brhos_createnew.php?ref_id_br=<?php echo $objResult["ref_id_br"];?>&start_date=<?php echo $_GET["start_date"];  ?>&end_date=<?php echo $_GET["end_date"];?>';}><img src="img/sticker.png" width="23" height="23" border="0" /></a>	
	<?php } ?>
</td>
 ...
 <div class="w3-panel">    <strong>พบทั้งหมด</strong>
	...
	if($Prev_Page)
	{
		echo " <a href='$_SERVER[SCRIPT_NAME]?Page=$Prev_Page&Keyword=$Keyword&start_date=$start_date&end_date=$end_date&sale_code1=$sale_code1'><font color='black'><< Back</font></a> ";
	}

	for($i=1; $i<=$Num_Pages; $i++){
		if($i != $Page)
		{
			echo "[ <a href='$_SERVER[SCRIPT_NAME]?Page=$i&Keyword=$Keyword&start_date=$start_date&end_date=$end_date&sale_code1=$sale_code1'><font color='black'>$i</font></a> ]";
	...
	if($Page!=$Num_Pages)
	{
		echo " <a href ='$_SERVER[SCRIPT_NAME]?Page=$Next_Page&Keyword=$Keyword&start_date=$start_date&end_date=$end_date&sale_code1=$sale_code1'><font color='black'>Next>></font></a> ";
	}
