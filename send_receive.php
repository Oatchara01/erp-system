...
$strSQL2 = "SELECT *  FROM tb_register_data  WHERE ref_id = '".$ref_id."' ";
...
echo "alert('รายการนี้ได้ส่งข้อมูลไปรับจ่ายบัญชีแล้วค่ะ');window.location='register_admin_edit.php?ref_id=$ref_id';";
echo "</script>";
exit();
  }
...
$delivery_date = $objResult["doc_release_date"];	
}	
...
cash,employee_name,ref_id,credit,bill_id,sale_channel) values ('".$doc_no."','".$delivery_date."','".$com."','".$billing_name."','".$transfer_date."','".$amount_1."','".$cash."','".$name."','".$ref_id."','".$credit."','".$bill_id."','".$sale_channel."')";
$objQuery29 = mysqli_query($code,$strSQL29);	
 ...
 if($objQuery29){
   echo "<script language=\"JavaScript\">";
echo "alert('ส่งข้อมูลไปรับจ่ายบัญชี เรียบร้อยแล้วค่ะ');window.location='register_admin_edit.php?ref_id=$ref_id';";
echo "</script>";
  } else {
   echo "Cannot";
  }
