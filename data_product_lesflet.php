...
while ($row = mysqli_fetch_array( $result )) {
	$express_name = $row["access_name"]; // ฟิลที่ต้องการส่งค่ากลับ
	$product_ID =$row["product_ID"];
	// ป้องกันเครื่องหมาย '
	$express_name = str_replace("'", "'", $express_name);
	// กำหนดตัวหนาให้กับคำที่มีการพิมพ์
	$display_name = preg_replace("/(" . $product_code_search . ")/iu", "<b>$1</b>", $express_name);
	echo "<li onselect=\"this.setText('$product_ID').setValue('$product_ID');\">$display_name</li>";
}
?>