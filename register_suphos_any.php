...
$bill_name = $rs12["cutomer"];	
	
$sql58 = "SELECT credit_ckk,credit_thb FROM tb_customer where customer_id  = '".$rs12["cutomer_id"]."'";
$qry58 = mysqli_query($conn,$sql58) or die(mysqli_error());
$rs58 = mysqli_fetch_assoc($qry58);
	
$sql59 = "select pay_in from tb_bank where close_ckk='0' and id  = '".$rs12["credit_ckk"]."'";
...
if (remain < 0 && creditThb !== 0) {
	...
	<form action='register_suphos_any1.php' method="post" name="frmMain" enctype="multipart/form-data" onSubmit="JavaScript:return fncSubmit();" >
...
<?php
  // ดึงค่ามาให้ชัด
  $bill_id = isset($rs12['customer_id']) ? trim((string)$rs12['customer_id']) : '';
?>
<?php if ($rs59['pay_in'] !== ''): ?>
...
<input type='hidden' name = "mode_name"  id = "mode_name"  value="<?php echo $rs12["mode_name"]; ?>" class="button4" readonly>	
...
<input type='text' name = "bill_name"  id = "bill_name" value="<?php echo $rs12["bill_name"]; ?>" style="width:30%;" class="button4" >
				</p>
...
<input type="checkbox" name="full_bill" value="1"> &nbsp;บิล VAT
 &nbsp;&nbsp;
เลขประจำตัวผู้เสียภาษี :&nbsp;&nbsp;&nbsp;<input type="text" name="tax_id" id ="tax_id"  value="<?php echo $rs12["tax_id"]; ?>" class="button4" style="width:22%;" >
</p>
...
<?php /*&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
Sales Comment :&nbsp;
<textarea name="sale_comment" id="sale_comment" class="button4" style="width:30%"   rows="2"></textarea>*/?></p>
...
กำหนดส่งตามสัญญา :
<input name="delivery_contract" type='date' id="delivery_contract" class="button4" style="width:30%" placeholder="กรุณาระบุเป็นวันที่เท่านั้น !!!"> </p>
