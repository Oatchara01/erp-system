...
$strSQL1 ="SELECT SUM(amount) as amount  FROM tb__buypro  WHERE type_arae = '1'  and group_pro = '".$objResult["group_pro"]."'";
...
$strSQL2 ="SELECT SUM(amount) as amount  FROM tb__buypro  WHERE type_arae = '2'  and group_pro = '".$objResult["group_pro"]."'";
...
$strSQL4 ="SELECT SUM(amount) as amount  FROM tb__buypro  WHERE type_arae = '3'  and group_pro = '".$objResult["group_pro"]."'";

if($start_date !=""){ 
    $strSQL4 .= ' AND doc_date >= "'.$start_date.'"'; 
}

if($end_date !=""){ 
    $strSQL4 .= ' AND doc_date <= "'.$end_date.'"'; 
}	
...
//ลดหนี้
	...
	$strSQL11 ="SELECT SUM(amount) as amount  FROM tb__discash  WHERE type_arae = '1'  and group_1 = '".$objResult["group_pro"]."'";
...
$strSQL12 ="SELECT SUM(amount) as amount  FROM tb__discash  WHERE type_arae = '2'  and group_1 = '".$objResult["group_pro"]."'";
...
$strSQL14 ="SELECT SUM(amount) as amount  FROM tb__discash  WHERE type_arae = '3'  and group_1 = '".$objResult["group_pro"]."'";

if($start_date !=""){ 
    $strSQL14 .= ' AND date_cash >= "'.$start_date.'"'; 
}

if($end_date !=""){ 
    $strSQL14 .= ' AND date_cash <= "'.$end_date.'"'; 
}	
...
$sum_disall = $sum_disall1;
	
$sum_disawl1 = $objResult1["amount"]-$objResult11["amount"];	
$sum_disawl = $sum_disawl1;

$sum_disoth1 = $objResult4["amount"]-$objResult14["amount"];	
$sum_disoth = $sum_disoth1;	
	
$sum_disnbm1 = $objResult2["amount"]-$objResult12["amount"];
$sum_disnbm = $sum_disnbm1;
	
$strSQL17 = "SELECT *  FROM tb__buypro_ser   WHERE group_1 = '".$objResult["group_pro"]."'";
$objQuery17 = mysqli_query($conn,$strSQL17) or die ("Error Query [".$strSQL17."]");
$Num_Rows17 = mysqli_num_rows($objQuery17);
if($Num_Rows17 > 0){

$strSQL71 = "UPDATE tb__buypro_ser SET sumary='".$sum_disall."',awl_sum='".$sum_disawl."',nbm_sum='".$sum_disnbm."',oth_sum='".$sum_disoth."' where group_1='".$objResult["group_pro"]."' ";
...
$strSQL71 = "insert into tb__buypro_ser
(group_1,company,sumary,awl_sum,nbm_sum,oth_sum)
values ('".$objResult["group_pro"]."','".$objResult["company"]."','".$sum_disall."','".$sum_disawl."','".$sum_disnbm."','".$sum_disoth."')";
...
<center>
<h3 align="center">รายงานยอดขายตามกลุ่มสินค้า</h3>
...
$strSQL13 ="SELECT SUM(sumary) as sumary,SUM(awl_sum) AS awl_sum,SUM(nbm_sum) AS nbm_sum,SUM(oth_sum) AS oth_sum  FROM tb__buypro_ser ";
...
$strSQL7 = "SELECT *  FROM tb__buypro_ser   Order by group_1 ASC ";
...
<td  align="right" class="style30"><?php echo  number_format($objResult7["awl_sum"],2).""; ?></td> 
<td  align="right" class="style30"><?php echo  number_format($objResult7["nbm_sum"],2).""; ?></td> 
<td  align="right" class="style30"><?php echo  number_format($objResult7["oth_sum"],2).""; ?></td> 
...
<td  align="right" class="style30"><?php echo  number_format($objResult13["awl_sum"],2).""; ?></td> 
<td  align="right" class="style30"><?php echo  number_format($objResult13["nbm_sum"],2).""; ?></td> 
<td  align="right" class="style30"><?php echo  number_format($objResult13["oth_sum"],2).""; ?></td> 
