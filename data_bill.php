...
$table_db=" hos__so"; // ตารางที่ต้องการค้นหา
...
while ($row = mysqli_fetch_array( $result )) {
	$bill_name = $row["bill_name"]; // ฟิลที่ต้องการส่งค่ากลับ
	// ป้องกันเครื่องหมาย '
	$bill_name = str_replace("'", "'", $bill_name);
	// กำหนดตัวหนาให้กับคำที่มีการพิมพ์
	$display_name = preg_replace("/(" . $product_code_search . ")/iu", "<b>$1</b>", $bill_name);
	echo "<li onselect=\"this.setText('$bill_name').setValue('$bill_name');\">$display_name</li>";
}
?>