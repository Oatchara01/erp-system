...
<div class="w3-container w3-bar w3-light-gray w3-margin-bottom"><h4>รายงานใบเบิกสินค้า SMP (ทั้งหมด)</h4></div>
...
$strSQL = "SELECT DISTINCT sale_code  FROM hos__smp  where status_sup='Approve' and chang_ckk='0'";
...
$strSQL41 = "SELECT DISTINCT ref_idsmp FROM (hos__smp LEFT JOIN hos__subsmp ON hos__subsmp.reff_idsmp=hos__smp.ref_idsmp)  where sale_code='".$objResult["sale_code"]."' and  status_sup='Approve' and chang_ckk='0'";

if($start_date !=""){ 
    $strSQL41 .= ' AND smp_date >= "'.$start_date.'"'; 
}

if($end_date !=""){ 
    $strSQL41 .= ' AND smp_date <= "'.$end_date.'"'; 
}
...
<a href="register_admin_edit.php?ref_id=<?php echo $objResult1["ref_idsale"];?>"  target="_blank"><?php echo $rs["order_id"];?></a></td>
...
$strSQL11 = "SELECT sol_name,sale_count,unit_name FROM (so__submain LEFT JOIN tb_product ON so__submain.product_ID=tb_product.product_id) WHERE ref_idd = '".$objResult1["ref_idsale"]."' ";
...
<td><?php 
	if($objResult1["dm_name"]!=''){
echo "(1)"; echo " ";	echo $objResult1["sup_name"]; ?><br>	
<?php echo "(2)"; echo " ";	echo $objResult1["dm_name"]; ?><br>	
	
<?php 	}else{
	
echo $objResult1["sup_name"]; } ?></td>
